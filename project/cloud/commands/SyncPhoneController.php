...
/**
 * 同步平台手机号
 * 本地测试：/usr/local/php7/bin/php /data/wwwroot/cloud/yii  sync-phone runPhone
 * D:\phpstudy\php55\php.exe  D:\phpstudy\WWW\cloud_ssdb\yii sync-phone runphone
 */
class SyncPhoneController extends BaseController
{   
    ...
    public function init()
    {
        ...
        $this->file_path = Yii::$app->basePath . '/../edge/';
        $this->field = 'user_phone,phone';
    }
    ...
    private function setRelationList(&$complex_phone_list){
        ...
        # check file and get path
        // if (SYSTEM_PROD) {
            $path = $this->getCheckPath();
        // } else {
        //     $path = $this->file_path.'20180905'.'/11111.csv';
        // }
        $fp = fopen($path,'a');
        clearstatcache(); # 清除缓存
        if (file_exists($path) && filesize($path) == 0) {
            $head = $this->field.PHP_EOL;
            fwrite($fp, $head);
        }
        $relation_list = [];
    ...
    private function getCheckPath(){
        $time = date("Ymd");
        # check dirname
        $dirname = $this->file_path.$time.'/';
        if (!is_dir($dirname)) {
            mkdir($dirname,0777,true);
        }
        # 获取最新的文件
        $shell_order = "ls -lt ".$dirname." | head -n 2 |awk '{print $9}'";
        $new_file = exec($shell_order);
        if (empty($new_file)) {
            $path = $dirname.'edge_'.date("His").'.csv';
            touch($path);
            chmod($path,0777);
        } else {
        ...
        # 最新文件中行数
        $line_shell_order = "cat ".$path." |wc -l";
        $lines_num = exec($line_shell_order);
        if ($lines_num >= 1000000) {
            $path = $dirname.'edge_'.date("His").'.csv';
            touch($path);
            chmod($path,0777);
        }
    ...
    private function setPhoneNum(&$complex_phone_list){
        ...
        Logger::dayLog('syncPhone/time','getPhoneList use_time:', $thistime1);
        # check phone
        $not_exist_phone_list = $this->getNotExistPhone($all_phone_list);
        $time3 = explode(' ',microtime());
        $thistime2 = $time3[0]+$time3[1]-($time2[0]+$time2[1]);
        Logger::dayLog('syncPhone/time','getNotExistPhone use_time:', $thistime2);
        # addAllPhoneNum
        if ($not_exist_phone_list) {
            $chunk_phone_list = array_chunk($not_exist_phone_list,500);
            for ($i=0; $i < count($chunk_phone_list) ; $i++) { 
                $set_res += $this->addAllPhoneNum($chunk_phone_list[$i]);
            }
        }
        $time4 = explode(' ',microtime());
        $thistime3 = $time4[0]+$time4[1]-($time3[0]+$time3[1]);
        Logger::dayLog('syncPhone/time','addAllPhoneNum use_time:', $thistime3);
        return $set_res;
    }
    ...
    {   
        try {
            $phone_num_list = array_map(function($phone) {
                                return [$phone];
                            },$phone_num_list);
            //数据批量入库  
            $res = self::$db_down->createCommand()->batchInsert(  
