...
class ArrayGroupBy
{
    ...
    public function groupByField($group_field)
    {
        ...
        $grouped = call_user_func_array("app\common\ArrayGroupBy::groupByFieldDeep", $group_by_field_params);
    ...
    {
        if (!is_array($arr)) {
            $message = 'app\common\ArrayGroupBy::groupByFieldDeep(): The first argument should be an array';
            throw new \InvalidArgumentException($message);
        }
        if (!is_string($key) && !is_int($key) && !is_float($key)) {
            $message = 'app\common\ArrayGroupBy::groupByFieldDeep(): The key should be a string or an integer';
            throw new \InvalidArgumentException($message);
        }
        ...
        if (func_num_args() > 3) {
            ...
            foreach ($grouped as $key => $value) {
                ...
                $grouped[$key] = call_user_func_array("app\common\ArrayGroupBy::groupByFieldDeep", $params);
            }
        }
        return $grouped;
    }
    ...
    public static function getDeepestArray($array)
    {
        ...
        foreach ($array as $arr) {
            if (!is_array($arr)) {
                ...
                $sub_result = call_user_func("app\common\ArrayGroupBy::getDeepestArray", $arr);
    ...
    public static function arrayColumn($array, $column_key, $index_key = null)
    {
        if (!is_array($array) && !($array instanceof \ArrayAccess))
            throw new \Exception('Argument 1 passed to app\common\ArrayGroupBy::arrayColumn() must be of the type array');
