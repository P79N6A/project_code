...
class Curl
{
    ...
    public function getHeader($key)
    {
        ...
        return is_array($headers) && isset($headers[$key]) ? $headers[$key] : NULL;
    }
    ...
    public static function dataGet($url, $timeout=60) {
        ...
        $http_status = curl_getinfo($curl_handle, CURLINFO_HTTP_CODE); //获取请求状态码
    ...
    //POST方式直接提交数据
    public static function dataPost($content, $url, $timeout=60) {
        // $context = array('http' => array('method' => "POST", 'header' => "Content-type: application/json", 'payload' => $content));
        ...
        $ret = curl_exec($ch);
        $http_status = curl_getinfo($ch, CURLINFO_HTTP_CODE); //获取请求状态码
        $ret_data = [
                'http_status' => $http_status,
                'result'=> $ret,
            ];
