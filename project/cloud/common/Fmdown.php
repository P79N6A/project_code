...
class Fmdown {
    ...
    private function runReport(&$report) {
        ...
        # device_info
        $device_info = ArrayHelper::getValue($report,'device_info',[]);
        # geoip_info
        $geoip_info = ArrayHelper::getValue($report,'geoip_info',[]);
        $data = [
            ...
            'geoip_info' => json_encode($geoip_info,JSON_UNESCAPED_UNICODE),
        ];
    ...
    private function reportEvent($v) {
        ...
        //     break;
        // }
        if (strstr($v, '借款策略集') !== false) {
            $event = 'loan_web';
        } elseif(strstr($v, '注册策略集') !== false) {
            $event = 'register_web';
        }      
        return $event;
    }
    ...
    private function analysisDetail(&$rules){
        ...
        // 设备状态异常_中风险
        $device_status_middle = $this->getRuleDetails($rules_info, [
            "设备状态异常_中风险",
        ]);
        // 设备状态异常_高风险
        $device_status_high = $this->getRuleDetails($rules_info, [
            "设备状态异常_高风险",
        ]);
        // 设备状态异常_设备首次出现
        $device_first_appear = $this->getRuleDetails($rules_info, [
            "设备状态异常_设备首次出现",
        ]);
        // ROOT识别
        $root_check = $this->getRuleDetails($rules_info, [
            "ROOT识别",
        ]);
        // 作弊工具识别
        $cheat_tool_check = $this->getRuleDetails($rules_info, [
            "作弊工具识别",
        ]);
        // 设备状态异常_疑似模拟定位
        $suspected_simulated_position = $this->getRuleDetails($rules_info, [
            "设备状态异常_疑似模拟定位",
        ]);
        // 设备标识缺失异常_没有传递设备指纹参数
        $no_blackbox_param = $this->getRuleDetails($rules_info, [
            "设备标识缺失异常_没有传递设备指纹参数",
        ]);
        // 设备标识缺失异常_高风险
        $device_lack_high = $this->getRuleDetails($rules_info, [
            "设备标识缺失异常_高风险",
        ]);
        // 设备标识缺失异常_中风险
        $device_lack_middle = $this->getRuleDetails($rules_info, [
            "设备标识缺失异常_中风险",
        ]);
        // 设备获取异常
        $device_get_abnormal = $this->getRuleDetails($rules_info, [
            "设备获取异常",
        ]);
        // 设备状态异常_监测到调试器
        $monitor_debugger = $this->getRuleDetails($rules_info, [
            "设备状态异常_监测到调试器",
        ]);
        // 越狱识别
        $break_prison = $this->getRuleDetails($rules_info, [
            "越狱识别",
        ]);
        // 设备缺失且使用国外或港澳台IP
        $not_inner_mesh_ip = $this->getRuleDetails($rules_info, [
            "设备缺失且使用国外或港澳台IP",
        ]);
        // 设备状态异常_虚假设备
        $false_equipment = $this->getRuleDetails($rules_info, [
            "设备状态异常_虚假设备",
        ]);
        // 登录设备缺失且使用国外或港澳台IP
        $reg_not_inner_mesh_ip = $this->getRuleDetails($rules_info, [
            "登录设备缺失且使用国外或港澳台IP",
        ]);
        // 安卓模拟器识别
        $android_blue_stacks = $this->getRuleDetails($rules_info, [
            "安卓模拟器识别",
        ]);
        // 1天内设备使用过多账户进行借款
        $one_device_account_num = $this->getRuleDetails($rules_info, [
            "1天内设备使用过多账户进行借款",
        ]);
        // 7天内设备使用过多账户进行借款
        $seven_device_account_num = $this->getRuleDetails($rules_info, [
            "7天内设备使用过多账户进行借款",
        ]);
        // 1天内设备使用过多的IP进行借款
        $one_device_ip_num = $this->getRuleDetails($rules_info, [
            "1天内设备使用过多的IP进行借款",
        ]);
        // 7天内设备使用过多的IP进行借款
        $seven_device_ip_num = $this->getRuleDetails($rules_info, [
            "7天内设备使用过多的IP进行借款",
        ]);
        // 1天内账户在过多的设备上进行借款
        $one_account_device_num = $this->getRuleDetails($rules_info, [
            "1天内账户在过多的设备上进行借款",
        ]);
        // 7天内账户在过多的设备上进行借款
        $seven_account_device_num = $this->getRuleDetails($rules_info, [
            "7天内账户在过多的设备上进行借款",
        ]);
        // 3个月内银行卡在多个平台进行借款
        $three_m_card_multi = $this->getRuleDetails($rules_info, [
            "3个月内银行卡在多个平台进行借款",
        ]);
        // 3个月内设备在多个平台进行借款
        $three_m_device_multi = $this->getRuleDetails($rules_info, [
            "3个月内设备在多个平台进行借款",
        ]);
        $data =  [
            // 逾期
            ...
            'user_device_danger_match_detail' => $user_device_danger_match,
             ### 新增设备数据详情
            // '设备状态异常_中风险'
            'device_status_middle_detail' => $device_status_middle,
            // '设备状态异常_高风险'
            'device_status_high_detail' => $device_status_high,
            // '设备状态异常_设备首次出现'
            'device_first_appear_detail' => $device_first_appear,
            // 'ROOT识别'
            'root_check_detail' => $root_check,
            // '作弊工具识别'
            'cheat_tool_check_detail' => $cheat_tool_check,
            // '设备状态异常_疑似模拟定位'
            'suspected_simulated_position_detail' => $suspected_simulated_position,
            // '设备标识缺失异常_没有传递设备指纹参数'
            'no_blackbox_param_detail' => $no_blackbox_param,
            // '设备标识缺失异常_高风险'
            'device_lack_high_detail' => $device_lack_high,
            // '设备标识缺失异常_中风险'
            'device_lack_middle_detail' => $device_lack_middle,
            // '设备获取异常'
            'device_get_abnormal_detail' => $device_get_abnormal,
            // '设备状态异常_监测到调试器'
            'monitor_debugger_detail' => $monitor_debugger,
            // '越狱识别'
            'break_prison_detail' => $break_prison,
            // '设备缺失且使用国外或港澳台IP'
            'not_inner_mesh_ip_detail' => $not_inner_mesh_ip,
            // '设备状态异常_虚假设备'
            'false_equipment_detail' => $false_equipment,
            // '登录设备缺失且使用国外或港澳台IP'
            'reg_not_inner_mesh_ip_detail' => $reg_not_inner_mesh_ip,
            // '安卓模拟器识别'
            'android_blue_stacks_detail' => $android_blue_stacks,
            // '1天内设备使用过多账户进行借款'
            'one_device_account_num_detail' => $one_device_account_num,
            // '7天内设备使用过多账户进行借款'
            'seven_device_account_num_detail' => $seven_device_account_num,
            // '1天内设备使用过多的IP进行借款' 
            'one_device_ip_num_detail' => $one_device_ip_num,
            // '7天内设备使用过多的IP进行借款'
            'seven_device_ip_num_detail' => $seven_device_ip_num,
            // '1天内账户在过多的设备上进行借款'
            'one_account_device_num_detail' => $one_account_device_num,
            // '7天内账户在过多的设备上进行借款'
            'seven_account_device_num_detail' => $seven_account_device_num,
            // '3个月内银行卡在多个平台进行借款'
            'three_m_card_multi_detail' => $three_m_card_multi,
            // '3个月内设备在多个平台进行借款'
            'three_m_device_multi_detail' => $three_m_device_multi,
        ]; 
        return $data;
    }
    ...
    private function myrulesAnalysis(&$myrules)
    {
        ...
        // 设备状态异常_中风险
        $device_status_middle = $this->detailAnalysis($rule_names, [
            "设备状态异常_中风险",
        ]);
        // 设备状态异常_高风险
        $device_status_high = $this->detailAnalysis($rule_names, [
            "设备状态异常_高风险",
        ]);
        // 设备状态异常_设备首次出现
        $device_first_appear = $this->detailAnalysis($rule_names, [
            "设备状态异常_设备首次出现",
        ]);
        // ROOT识别
        $root_check = $this->detailAnalysis($rule_names, [
            "ROOT识别",
        ]);
        // 作弊工具识别
        $cheat_tool_check = $this->detailAnalysis($rule_names, [
            "作弊工具识别",
        ]);
        // 设备状态异常_疑似模拟定位
        $suspected_simulated_position = $this->detailAnalysis($rule_names, [
            "设备状态异常_疑似模拟定位",
        ]);
        // 设备标识缺失异常_没有传递设备指纹参数
        $no_blackbox_param = $this->detailAnalysis($rule_names, [
            "设备标识缺失异常_没有传递设备指纹参数",
        ]);
        // 设备标识缺失异常_高风险
        $device_lack_high = $this->detailAnalysis($rule_names, [
            "设备标识缺失异常_高风险",
        ]);
        // 设备标识缺失异常_中风险
        $device_lack_middle = $this->detailAnalysis($rule_names, [
            "设备标识缺失异常_中风险",
        ]);
        // 设备获取异常
        $device_get_abnormal = $this->detailAnalysis($rule_names, [
            "设备获取异常",
        ]);
        // 设备状态异常_监测到调试器
        $monitor_debugger = $this->detailAnalysis($rule_names, [
            "设备状态异常_监测到调试器",
        ]);
        // 越狱识别
        $break_prison = $this->detailAnalysis($rule_names, [
            "越狱识别",
        ]);
        // 设备缺失且使用国外或港澳台IP
        $not_inner_mesh_ip = $this->detailAnalysis($rule_names, [
            "设备缺失且使用国外或港澳台IP",
        ]);
        // 设备状态异常_虚假设备
        $false_equipment = $this->detailAnalysis($rule_names, [
            "设备状态异常_虚假设备",
        ]);
        // 登录设备缺失且使用国外或港澳台IP
        $reg_not_inner_mesh_ip = $this->detailAnalysis($rule_names, [
            "登录设备缺失且使用国外或港澳台IP",
        ]);
        // 安卓模拟器识别
        $android_blue_stacks = $this->detailAnalysis($rule_names, [
            "安卓模拟器识别",
        ]);
        // 1天内设备使用过多账户进行借款
        $one_device_account_num = $this->detailAnalysisNum($myrules, [
            "1天内设备使用过多账户进行借款",
        ]);
        // 7天内设备使用过多账户进行借款
        $seven_device_account_num = $this->detailAnalysisNum($myrules, [
            "7天内设备使用过多账户进行借款",
        ]);
        // 1天内设备使用过多的IP进行借款
        $one_device_ip_num = $this->detailAnalysisNum($myrules, [
            "1天内设备使用过多的IP进行借款",
        ]);
        // 7天内设备使用过多的IP进行借款
        $seven_device_ip_num = $this->detailAnalysisNum($myrules, [
            "7天内设备使用过多的IP进行借款",
        ]);
        // 1天内账户在过多的设备上进行借款
        $one_account_device_num = $this->detailAnalysisNum($myrules, [
            "1天内账户在过多的设备上进行借款",
        ]);
        // 7天内账户在过多的设备上进行借款
        $seven_account_device_num = $this->detailAnalysisNum($myrules, [
            "7天内账户在过多的设备上进行借款",
        ]);
        // 3个月内银行卡在多个平台进行借款
        $three_m_card_multi = $this->detailAnalysisNum($myrules, [
            "3个月内银行卡在多个平台进行借款",
        ]);
        // 3个月内设备在多个平台进行借款
        $three_m_device_multi = $this->detailAnalysisNum($myrules, [
            "3个月内设备在多个平台进行借款",
        ]);
        //新多投
        $data = [
            ...
            ### 新增设备数据
            // '设备状态异常_中风险'
            'device_status_middle' => $device_status_middle,
            // '设备状态异常_高风险'
            'device_status_high' => $device_status_high,
            // '设备状态异常_设备首次出现'
            'device_first_appear' => $device_first_appear,
            // 'ROOT识别'
            'root_check' => $root_check,
            // '作弊工具识别'
            'cheat_tool_check' => $cheat_tool_check,
            // '设备状态异常_疑似模拟定位'
            'suspected_simulated_position' => $suspected_simulated_position,
            // '设备标识缺失异常_没有传递设备指纹参数'
            'no_blackbox_param' => $no_blackbox_param,
            // '设备标识缺失异常_高风险'
            'device_lack_high' => $device_lack_high,
            // '设备标识缺失异常_中风险'
            'device_lack_middle' => $device_lack_middle,
            // '设备获取异常'
            'device_get_abnormal' => $device_get_abnormal,
            // '设备状态异常_监测到调试器'
            'monitor_debugger' => $monitor_debugger,
            // '越狱识别'
            'break_prison' => $break_prison,
            // '设备缺失且使用国外或港澳台IP'
            'not_inner_mesh_ip' => $not_inner_mesh_ip,
            // '设备状态异常_虚假设备'
            'false_equipment' => $false_equipment,
            // '登录设备缺失且使用国外或港澳台IP'
            'reg_not_inner_mesh_ip' => $reg_not_inner_mesh_ip,
            // '安卓模拟器识别'
            'android_blue_stacks' => $android_blue_stacks,
            // '1天内设备使用过多账户进行借款'
            'one_device_account_num' => $one_device_account_num,
            // '7天内设备使用过多账户进行借款'
            'seven_device_account_num' => $seven_device_account_num,
            // '1天内设备使用过多的IP进行借款' 
            'one_device_ip_num' => $one_device_ip_num,
            // '7天内设备使用过多的IP进行借款'
            'seven_device_ip_num' => $seven_device_ip_num,
            // '1天内账户在过多的设备上进行借款'
            'one_account_device_num' => $one_account_device_num,
            // '7天内账户在过多的设备上进行借款'
            'seven_account_device_num' => $seven_account_device_num,
            // '3个月内银行卡在多个平台进行借款'
            'three_m_card_multi' => $three_m_card_multi,
            // '3个月内设备在多个平台进行借款'
            'three_m_device_multi' => $three_m_device_multi,
        ];
        return $data;
    }
    ...
    private function getMyrules(&$rule)
    {
        ...
        if (in_array($name, $multi_rules)) {
            ...
            if (strstr($name, '设备使用过多账户进行借款') !== false ) {
                $num = ArrayHelper::getValue($rule, "规则详情.0.设备ID关联借款人手机数目", 0);
            } elseif (strstr($name, '设备使用过多的IP进行借款') !== false ) {
                $num = ArrayHelper::getValue($rule, "规则详情.0.设备ID关联IP3数目", 0);
            } elseif(strstr($name,'3个月内设备在多个平台进行借款') !== false) {
                $num = ArrayHelper::getValue($rule, "规则详情.1.个数", 0);
            } else {
