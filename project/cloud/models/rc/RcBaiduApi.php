...
class RcBaiduApi {
    ...
    {      
        ...
        $apiType = 'LBS';
        //请求前入库
        $oRcBaidulbs = new RcBaidulbs();
        $save_lbs = $oRcBaidulbs->saveData($data,$params['reqid']);
        if (!$save_lbs) {
            Logger::dayLog('BaiduLbs/save_lbs', '百度请求保存失败', $params,$oRcBaidulbs->errors);
        }
        //请求接口
        $baidu_lbs = $this->sendRequest($params,$apiType);
        //LBS结果更新
        $updata_lbs = $oRcBaidulbs->updateLbsInfo($baidu_lbs);
        if (!$updata_lbs) {
            Logger::dayLog('BaiduLbs/updata_lbs', '百度请求更新失败', $params,$oRcBaidulbs->errors);
        }
        return $baidu_lbs;
    }
