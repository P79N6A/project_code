...
class CloudController extends ApiController 
{
    ...
    /**
     * [actionOrigin 新百度LBS接口]
     * @return [type] [description]
     */
    public function actionNewBaidulbs()
    {
        $postdata = $this->postdata;
        if (!is_array($postdata) || empty($postdata)) {
            Logger::dayLog('origin', 'postdata', '数据异常', $postdata);
        ...
        if (!isset($postdata['mobile']) || !isset($postdata['identity']) || !isset($postdata['user_id']) || !isset($postdata['aid'])|| !isset($postdata['latitude']) || !isset($postdata['longtitude']) || !isset($postdata['location'])) {
            Logger::dayLog('origin', 'postdata', '参数缺失', $postdata);
            return $this->error('20002', '参数缺失', $postdata);
        }
        $api = new XsApi();
        $res = $api->getNewBaiduLbs($postdata);
        if (isset($res['res_code']) && $res['res_code'] != '0') {
            return $this->error($res['res_code'], $res['res_data'], $postdata);
        }
