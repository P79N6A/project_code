...
class Addrloan(object):
    ...
    def chunk(self, biglist):
        ...
        for i in range(0,int(mc)):
            lst.append(mobiles[i*limit:(i+1)*limit])
    ...
    def run(self):
        ...
        realadl_tot_reject_num = history_bad_status_total.get('realadl_tot_reject_num') if history_bad_status_total else None
        realadl_tot_freject_num = history_bad_status_total.get('realadl_tot_freject_num') if history_bad_status_total else None
        realadl_tot_sreject_num = history_bad_status_total.get('realadl_tot_sreject_num') if history_bad_status_total else None
        realadl_tot_dlq14_num = history_bad_status_total.get('realadl_tot_dlq14_num') if history_bad_status_total else None
        realadl_dlq14_ratio = history_bad_status_total.get('realadl_dlq14_ratio') if history_bad_status_total else None
        history_bad_status = history_bad_status_total.get('history_bad_status') if history_bad_status_total else None
        ...
        info =  {
            ...
            'history_bad_status':history_bad_status
        }
        return info
