...
class peanutAndroidPageNameModel(models.Model):
    ...
    class Meta:
        db_table ="pea_android_page_name"
    ...
    def getUrlList(self, pageId = -1):
        if pageId == -1:
            return peanutAndroidPageNameModel.objects.values('id', 'page_name_id', 'page_name').all()
        else:
            return peanutAndroidPageNameModel.objects.filter(page_name_id = pageId).values('id', 'page_name_id', 'page_name').all()

    def getPageIds(self, pageNames):
        pageIds = peanutAndroidPageNameModel.objects.filter(page_name__in=pageNames).values('page_name_id', 'page_name').all()
        pageList = {}
        pageNameList = []
        if len(pageIds) != 0:
    ...
    def pageNameSave(self, page_name):
        ...
        ob = peanutAndroidPageNameModel(page_name_id=0,page_name=page_name,create_time=now)
        ob.save()

    def setPageId(self, valueId, pageId):
        setResult = peanutAndroidPageNameModel.objects.filter(id = pageId).update(page_name_id = valueId)
