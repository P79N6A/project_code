...
class peanutBrowseModel(models.Model):
    ...
    def getBrowseNum(self, timeRange = None):
        ...
        data = peanutBrowseModel.objects.filter(data__range=timeRange).aggregate(total=models.Sum('browse_num'))
    ...
    def getBrowseData(self, timeRange = None, pageId = -1, equipment = -1):
        ...
        if pageId == -1 and equipment == -1:
            data = peanutBrowseModel.objects.filter(data__range=timeRange).order_by('-create_time').all()
        elif pageId != -1 and equipment == -1:
            data = peanutBrowseModel.objects.filter(data__range=timeRange, page_name=pageId).order_by('-create_time').all()
        elif pageId == -1 and equipment != -1:
            data = peanutBrowseModel.objects.filter(data__range=timeRange, equipment=equipment).order_by('-create_time').all()
        else:
            data = peanutBrowseModel.objects.filter(data__range=timeRange, equipment=equipment, page_name=pageId).order_by('-create_time').all()
        return data

    # 获取数据中最大时间与最大时间下一小时的时间
    def getMaxtime(self, equipment):
        lastRow = peanutBrowseModel.objects.filter(equipment=equipment).values('data').order_by('-data').first()
    ...
    def saveBrowse(self, page_name, browse_num, equipment, data):
        haveData = peanutBrowseModel.objects.filter(data=data, equipment=equipment, page_name=page_name, browse_num=browse_num).count()
        if haveData == 0:
            peanutBrowseModel(
                page_name = page_name,
