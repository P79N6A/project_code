...
class yyyAndroidPageNameModel(models.Model):
    ...
    class Meta:
        db_table ="yyy_android_page_name"
    ...
    def getUrlList(self, pageId = -1):
        if pageId == -1:
            return yyyAndroidPageNameModel.objects.values('id', 'page_name_id', 'page_name').all()
        else:
            return yyyAndroidPageNameModel.objects.filter(page_name_id = pageId).values('id', 'page_name_id', 'page_name').all()

    def getPageIds(self, pageNames):
        pageIds = yyyAndroidPageNameModel.objects.filter(page_name__in=pageNames).values('page_name_id', 'page_name').all()
        pageList = {}
        pageNameList = []
        if len(pageIds) != 0:
    ...
    def pageNameSave(self,page_name):
        ...
        ob = yyyAndroidPageNameModel(page_name_id=0,page_name=page_name,create_time=now)
        ob.save()

    def setPageId(self, valueId, pageId):
        setResult = yyyAndroidPageNameModel.objects.filter(id = pageId).update(page_name_id = valueId)
