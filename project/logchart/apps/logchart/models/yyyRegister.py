...
class yyyRegisterModel(models.Model):
    ...
    def getRegisterNum(self, timeRange):
        if not timeRange:
            timeRange = ['2000-01-01 00:00:00', '2100-12-31 23:59:59']
        data = yyyRegisterModel.objects.filter(data__range=timeRange).aggregate(total=models.Sum('register_num'))
    ...
    def getEquipRegister(self, timeRange=None):
        if not timeRange:
            timeRange = ['2000-01-01 00:00:00', '2100-12-31 23:59:59']
        data = yyyRegisterModel.objects.values('equipment').filter(data__range=timeRange).annotate(total=models.Sum('register_num'))
        return data

    # 获取最近的插入时间
    def getMaxtime(self, equipment):
        lastRow = yyyRegisterModel.objects.filter(equipment=equipment).values('data').order_by('-data').first()
