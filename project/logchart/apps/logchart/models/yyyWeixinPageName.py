...
class yyyWeixinPageNameModel(models.Model):
    ...
    def getUrlList(self, pageId = -1):
        if pageId == -1:
            return yyyWeixinPageNameModel.objects.values('id', 'page_name_id', 'page_name').all()
        else:
            return yyyWeixinPageNameModel.objects.filter(page_name_id = pageId).values('id', 'page_name_id', 'page_name').all()

    def getPageIds(self, pageNames):
        pageIds = yyyWeixinPageNameModel.objects.filter(page_name__in = pageNames).values('page_name_id', 'page_name').all()
        pageList = {}
        pageNameList = []
        if len(pageIds) != 0:
    ...
    def pageNameSave(self,page_name):
        ...
        ob = yyyWeixinPageNameModel(page_name_id=0,page_name=page_name,create_time=now)
        ob.save()

    def setPageId(self, valueId, pageId):
        setResult = yyyWeixinPageNameModel.objects.filter(id = pageId).update(page_name_id = valueId)
