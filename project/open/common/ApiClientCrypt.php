...
class ApiClientCrypt extends ApiCrypt
{
	 ...
	 * 先花花开发平台地址
	 */
	public $xhApiDomain='http://open.xianhuahua.com/api/';
	...
	// 向开发平台发送数据包
	public function sent($path, $data,$hostMap=null){
		$url  = $this->buildUrl($path);
		$data = $this->buildRequest($data);
		$request = ['app_id'=> $this->app_id, 'data'=>$data];
		return $this->curlByHost($url, $request, $hostMap);
	}
	// 组合api链接地址
	 ...
	 */
	private function curlByHost($url, $data, $hostMap=null ,$httpHeader=null){
		if( is_array($hostMap) ){
			// 获取host,ip映射关系
