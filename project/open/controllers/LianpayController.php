...
class LianpayController extends BaseController {
    ...
    public function actionBackbind($lianid = null) {
        ...
        //2. 签名处理
        if ($status == '0000') {
            $verify = $this->oCLian->verifyJson($result);
            if (!$verify) {
                return $this->showMessage(140402, '签名请求');
            }
        }
    ...
    public function actionBackpay() {
        ...
        //2. 验签
        $verify = $this->oCLian->verifyJson($data_json);
        if (!$verify) {
            return $this->showMessage(140410, '非法请求');
        }

        //3 解析数据; 保存状态. 通知结果
        $data = json_decode($data_json, true);
        $oCBack = new CBack;
        $result = $oCBack->backpay($data);
