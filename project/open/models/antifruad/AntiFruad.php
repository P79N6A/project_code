...
namespace app\models\antifruad;
 ...
 */
class AntiFruad extends BaseModel
{
    // 抓取状态
    ...
    public function rules()
    {
        return [
            ...
            [['status', 'user_id','aid','risk_score','found','id_found'], 'integer'],
            [['name'], 'string', 'max' => 50],
            [['error_msg'], 'string', 'max' => 255],
            [['risk_info'], 'string', 'max' => 2000],
            [['idcardno','mobile'], 'string', 'max' => 50],
        ];
    }
...
//        $str = json_encode($rst,JSON_UNESCAPED_UNICODE);//第二个参数汉子不转译
        ...
        $res->id_found = ArrayHelper::getValue($rst,'idFound','');
        $res->found = ArrayHelper::getValue($rst,'found','');
        $res->risk_info = json_encode( ArrayHelper::getValue($rst,'riskInfo',''));
        $res->risk_score = ArrayHelper::getValue($rst,'riskScore','');
        $result = $res->update();
//        print_r($res->getErrors());
        return $result;
    }

    public function oneErrorSave($id,$rst){
        if(empty($id) || empty($rst)){
            return false;
        }
        $res = $this->getOne($id);
        ...
        $res->modify_time = date('Y-m-d H:i:s');
        $res->error_msg = ArrayHelper::getValue($rst,'message','');
        $res->id_found = ArrayHelper::getValue($rst,'idFound','');
        $res->found = ArrayHelper::getValue($rst,'found','');
        $res->risk_info = json_encode( ArrayHelper::getValue($rst,'riskInfo',''));
        $res->risk_score = ArrayHelper::getValue($rst,'riskScore','');
        $result = $res->update();
//        print_r($res->getErrors());die;
        return $result;
    }
    ...
    public function isRepeatQuery($idCardno,$number = 30){
         ...
         * true  表示 需要重新验证
         * */
        $data = static::find()->where(['and',['idcardno'=>$idCardno],['=','status',static::STATUS_SUCCESS]])->orderBy('create_time desc')->asArray()->one();
        if(empty($data)){
            return true;
        }
        //状态
