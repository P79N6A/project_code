 ...
 */
class Baiduhc extends BaseModel
{
    // 抓取状态
    ...
    {
        return [
            [['user_id','status','create_time','reqid','aid'], 'required'],
            [['create_time','modify_time'], 'safe'],
           // [['status', 'user_id','aid','channel_id'], 'integer'],
            [['name'], 'string', 'max' => 50],
            [['reason'], 'string', 'max' => 2000],
            [['identity','mobile'], 'string', 'max' => 50],
        ];
    }
     ...
     * @param $rst //百度返回信息
     * */
    public function oneSave($data,$rst){
        if(empty($data) || empty($rst)){
            return false;
        }

        $id = ArrayHelper::getValue($data,'reqid');
        $res = $this->getOne($id);
        ...
        if($zero==0){
            ...
            Logger::dayLog('baiduhc','Request/error','数据不全：',$rst);
        }
     ...
     * 根据reqid查询单条语句
     * */
    public function getOne($id,$column='reqid'){
        if(empty($id)){
            return false;
        }
    ...
    public function isRepeatQuery($identity,$phone,$number = 90){
         ...
         * true  表示 需要重新验证
         * */
        $data = static::find()->where(['and',['identity'=>$identity,'mobile'=>$phone],['=','status',static::STATUS_SUCCESS]])->orderBy('create_time desc')->one();
        if(empty($data)){
            return true;
        }
        //状态
