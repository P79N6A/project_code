 ...
 */
class BfRemit extends \app\models\BaseModel {
    ...
    public function attributeLabels() {
        return ['id' => '结算记录ID', 'aid' => '应用id', 'channel_id'=>'通道id', 'req_id' => '请求ID(业务)', 'client_id' => '[系统生成]流水号(内部对宝付)','bf_orderid'=>'宝付订单号' ,'settle_amount' => '[必填]结算金额', 'settle_fee' => '结算手续费', 'real_amount' => '实际划款金额（除去手续费）', 'remit_type' => '[必填]打款业务类型：1表示借款；2担保提现；3收益提现', 'remit_status' => '打款状态:0:初始化;1:出款请求中;3:受理中;4:查询请求中;6:成功;11:失败;12:无响应(预留)', 'rsp_status' => '宝付:响应状态:空为新加, RSP_TIMEOUT表示无响应', 'rsp_status_text' => '宝付:响应结果', 'identityid' => '[选填]用户身份证', 'user_mobile' => '[选填]用户手机', 'guest_account_name' => '[必填]帐号名称(持卡人姓名)', 'account_type' => '账户类型:0对私；1对公 目前全是私', 'guest_account_bank' => '[收款]开户行名称', 'guest_account' => '[必填]银行账号', 'guest_account_province' => '[收款]银行所属省', 'guest_account_city' => '[收款]银行所属市', 'guest_account_bank_branch' => '[收款]银行所属支行', 'settlement_desc' => '[选填]结算描述信息', 'create_time' => '创建时间', 'modify_time' => '更新时间',
            'remit_time' => '出款时间', 'sub_remit_time' => '请求宝付出款时间',
    ...
    public function saveRemitData($postData) {
        ...
        $data = [
            ...
            'client_id' => $client_id,
            'bf_orderid'=>isset($postData['bf_orderid']) ? $postData['bf_orderid'] : '',
...
//        print_r($data);
    ...
    public function lockOneRemit(){
        try{
            ...
            Logger::dayLog('bfremit/lock', 'lockOneRemit',$this->id,'fail');
            $result = false;
        }
        return $result;
    }
    ...
    public function lockOneQuery(){
        try{
            ...
            Logger::dayLog('bfremit/lock', 'lockOneQuery', $this->id,'fail');
            $result = false;
        }
        return $result;
    }
