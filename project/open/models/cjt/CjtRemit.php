...
/**
 * This is the model class for table "rt_remit".
 *
 * @property string $id
 ...
 */
class CjtRemit extends \app\models\BaseModel {
    ...
    public function rules() {
        return [
            [
                ...
                ['aid', 'channel_id', 'remit_type', 'remit_status', 'account_type', 'card_type', 'query_num', 'version'], 'integer'], [
                ...
                ['rsp_status','cj_orderid'], 'string', 'max' => 50], [
    ...
    public function attributeLabels() {
        return ['id' => '结算记录ID', 'aid' => '应用id', 'channel_id'=>'通道id', 'req_id' => '请求ID(业务)', 'client_id' => '[系统生成]流水号(内部对畅捷)','cj_orderid'=>'畅捷订单号' ,'settle_amount' => '[必填]结算金额', 'settle_fee' => '结算手续费', 'real_amount' => '实际划款金额（除去手续费）', 'remit_type' => '[必填]打款业务类型：1表示借款；2担保提现；3收益提现', 'remit_status' => '打款状态:0:初始化;1:出款请求中;3:受理中;4:查询请求中;6:成功;11:失败;12:无响应(预留)', 'rsp_status' => '畅捷:响应状态:空为新加, RSP_TIMEOUT表示无响应', 'rsp_status_text' => '畅捷:响应结果', 'identityid' => '[选填]用户身份证', 'user_mobile' => '[选填]用户手机', 'guest_account_name' => '[必填]帐号名称(持卡人姓名)', 'account_type' => '账户类型:0对私；1对公 目前全是私', 'guest_account_bank' => '[收款]开户行名称', 'guest_account' => '[必填]银行账号', 'guest_account_province' => '[收款]银行所属省', 'guest_account_city' => '[收款]银行所属市', 'guest_account_bank_branch' => '[收款]银行所属支行', 'settlement_desc' => '[选填]结算描述信息', 'create_time' => '创建时间', 'modify_time' => '更新时间',
            'remit_time' => '出款时间', 'sub_remit_time' => '请求畅捷出款时间', 'card_type' => '银行卡类型',
    ...
    public function saveRemitData($postData) {
        ...
        //2  保存数据
        $datetime  = date('Y-m-d H:i:s'); // 组合
        $client_id = $this->getClientId($postData['channel_id']);
        $data = [
            'aid'                       => $postData['aid'],
            'channel_id'                =>$postData['channel_id'],
            'req_id'                    => $postData['req_id'],
            'client_id'                 => $client_id,
            'cj_orderid'                =>isset($postData['cj_orderid']) ? $postData['cj_orderid'] : '',
    ...
    public function getRemitByReqid($aid, $channel_id, $reqId) {
        $where = ['aid' => $aid, 'req_id' => $reqId, 'channel_id' =>$channel_id];
     ...
     * 生成出款流水号
     * */
    public function getClientId($channel_id) {
        $time = date('YmdHis', time());
        $str = rand(1000, 99999);
        $clientId = "R" . $time . $str;
        $where = ['channel_id' => $channel_id, 'client_id' => $clientId];
        $ret = static::find()->where($where)->count();
        if ($ret > 0) {
            $clientId = $this->getClientId($channel_id);
        }
        return $clientId;
    }
     ...
     * @param $rsp_status 畅捷接口响应状态
     * @param $rsp_status_text 畅捷接口响应结果
     * @param $sub_remit_time 提交畅捷出款时间
     * @param $cj_orderid 畅捷订单号
     * @param $source 1: 出款调用; 2:查询调用;3:异步通知调用
     * @return bool
     */
    public function saveRspStatus($remit_status, $rsp_status, $rsp_status_text, $sub_remit_time,$cj_orderid, $source) {
        // 当是出款中时, 更新下次的查询时间
        if ($source == 2) {
            // 累加查询次数
            $this->query_num++;
        ...
        if($source == 1 && $cj_orderid){
            $this->cj_orderid = (string)$cj_orderid;
        }
    ...
    public function getInitData($limit, $channel_id) {
        $where = ['AND', ['remit_status' => static::STATUS_INIT, 'channel_id' => $channel_id],
...
//        print_r($data);
     ...
     * 获取正在处理中的数据
     */
    public function getDoingData($limit, $channel_id) {
        $where = ['AND', ['remit_status' => static::STATUS_DOING, 'channel_id' => $channel_id],
    ...
    public function lockOneRemit(){
        try{
            ...
            Logger::dayLog('cjremit/lock', 'lockOneRemit',$this->id,'fail');
            $result = false;
        }
        return $result;
    }
    ...
    public function lockOneQuery(){
        try{
            ...
            Logger::dayLog('cjremit/lock', 'lockOneQuery', $this->id,'fail');
            $result = false;
        }
        return $result;
    }
...
//    private function isLimit($remit_type, $num) {
     ...
     * @return void
     */
    public function getInitRemit($postData) {
        $where = [
                    'remit_status' => static::STATUS_INIT,
                    'channel_id' => $postData['channel_id'],
                    'req_id' => $postData['req_id']
                ];
        $data = static::find()->where($where)->orderBy('create_time ASC')->limit(1)->one();
        return $data;
    }
