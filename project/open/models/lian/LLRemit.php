 ...
 */
class LLRemit extends \app\models\BaseModel {
    ...
    public function rules() {
        return [
            [
                ['aid', 'req_id', 'client_id', 'remit_status', 'remit_type', 'identityid', 'user_mobile', 'guest_account_name', 'guest_account_bank', 'guest_account_province', 'guest_account_city', 'guest_account', 'create_time', 'modify_time', 'callbackurl'], 'required'], [
                ['aid', 'batch_no', 'remit_type', 'remit_status', 'account_type', 'query_num', 'version'], 'integer'], [
                ['settle_amount', 'settle_fee', 'real_amount'], 'number'], [
                ['create_time', 'modify_time', 'remit_time', 'query_time', 'sub_remit_time'], 'safe'], [
                ['req_id','ll_orderid'], 'string', 'max' => 40], [
    ...
    public function saveRemitData($postData) {
        ...
        $data = [
            ...
            'client_id' => $client_id,
            'll_orderid' => isset($postData['ll_orderid']) ? $postData['ll_orderid'] : '',
     ...
     * @param $rsp_status 连连接口响应状态
     * @param $rsp_status_text 连连接口响应结果
     * @param $oid_paybill 连连支付订单号
     * @param $confirm_code 连连接口响应结果
     * @param $sub_remit_time 提交连连出款时间
     * @param $source 1: 出款调用; 2:查询调用;3:异步通知调用
     * @return bool
     */
    public function saveRspStatus($remit_status, $rsp_status, $rsp_status_text,$oid_paybill,$confirm_code, $sub_remit_time, $source) {
       // 当是出款中时, 更新下次的查询时间
        if ($source == 2) {
            // 累加查询次数
            $this->query_num++;
        ...
        if(!empty($oid_paybill)){
            $this->ll_orderid = $oid_paybill;
        }
...
//        print_r($data);
    ...
    public function InputNotify(LLRemit $oRemit)
    {
    ...
    private function addNotify(LLRemit $oRemit)
    {
        if (in_array($oRemit['remit_status'], [LLRemit::STATUS_SUCCESS, LLRemit::STATUS_FAILURE])) {
            ...
            if (!$result) {
                Logger::dayLog('lianback', 'LLRemit/addNotify', 'LLClientNotify/saveData', $oClientNotify->errors);
                return false;
            }
        }
        return true;
     ...
     * 判断是否为终态
     * @param LLRemit $oRemit
     * @return boolean
     */
    public static function isFinished(LLRemit $oRemit){
        if (in_array($oRemit['remit_status'], [LLRemit::STATUS_SUCCESS, LLRemit::STATUS_FAILURE])) {
            return true;
        }
        return false;
