...
class Rbremit
{
    ...
    {
        $res  = [];
        $channel_ids =[105,110,112,152,168,176,184];
        ...
        foreach ($remitData as $key => $oRemit) {
            ...
            if ($result) {
                ...
                if($result){
                    ...
                    Logger::dayLog('rbremit', 'Rbremit/runRemits', '保存batch_id失败', $oRemit->errors);
                }
            } else {
                $res = $oRemit->saveRspStatus(Remit::STATUS_FAILURE, '_ERROR', '规则出款限制', '', 1);
                if (!$res) {
                    Logger::dayLog('rongbao', 'RbRemit/saveRspStatus', $oRemit->errors);
                }
                $this->addNotify($oRemit);
                Logger::dayLog('rbremit', 'Rbremit/runRemits', '处理失败', $oRemit);
            }
            $batch_no = $oRemit['id'];
        }
...
//        Logger::dayLog(
        //                'rongbao', '请求日志', $batch_no, $post_data
        //        );
    ...
    private function saveStatus($remitData, $result, $sub_remit_time = '')
    {
        ...
        if ($result->result_code == '0000') {
            ...
            foreach ($remitData as $key => $oRemit) {
                ...
                if (!$res) {
                    Logger::dayLog('rongbao', 'CRemit/doRemit', 'Remit/saveRspStatus', $oRemit->errors);
                }
                $this->addNotify($oRemit);
            }
        }
...
//        1,210302196001012114,韩梅梅,建设银行,分行,支行,私,2,CNY,北京,北京,13220482188,身份证,210302196001012114,jk-07253dd,xh1234,test
     ...
     * 暂定每分钟最多跑20个
     */
    public function runBatchQuerys()
    {
        //1 一次性处理最大设置为10
        $initRet   = ['total' => 0, 'success' => 0];
        $restNum   = 50;
        $oRemit    = new Remit;
        $remitData = $oRemit->getDoingBatchData($restNum);
        ...
        //3 锁定状态为查询中
        $batch_nos = ArrayHelper::getColumn($remitData, 'batch_no');
        $allRemits = $oRemit->getDoingByBatchData($batch_nos);
        $ids       = ArrayHelper::getColumn($allRemits, 'id');
//        $ups = $oRemit->lockQuery($ids); // 锁定出款接口的请求
        ...
        foreach ($remitData as $oRemit) {
            $oRemit->refresh();
            $result = $this->doBatchQuery($oRemit);
            if ($result) {
                $success++;
            } else {
     ...
     * @return bool
     */
    /*private function doBatchQuery($oRemit) {
    //1 参数验证
    if (!$oRemit) {
    return false;
    }
    if (!isset($oRemit['batch_no']) || !$oRemit['batch_no']) {
    return false;
    }

    //2 纪录到请求日志中
    Logger::dayLog('rongbao_select_batch', $oRemit);
    //        $oApiLog = new ApiLog;
    ...
    //        }
    //5 提交到接口中并解析响应结果
    $response = $this->getApi($oRemit['aid'])->batchpayqueryApply($oRemit);
    Logger::dayLog('rongbao_select_batch', $response);

    //5.1 解析状态响应码
    $oRemitStatus = new RemitStatus();
    $result = $oRemitStatus->parseBatchStatus(json_decode(response));
    if (!$result) {
    return false;
    }
    //5.2 保存查询表中
    ...
    private function doQuery($oRemit)
    {
        ...
        $result = $oRemit->saveRspStatus($oRemitStatus->remit_status, $oRemitStatus->rsp_status, $oRemitStatus->rsp_status_text, '', 2);
        if (!$result) {
            Logger::dayLog('rongbao/select', 'Remit/saveRspStatus', $oRemit->id, $oRemit->errors);
            return false;
        }
    ...
    private function addNotify(Remit $oRemit)
    {
        if (in_array($oRemit['remit_status'], [Remit::STATUS_SUCCESS, Remit::STATUS_FAILURE])) {
            ...
            if (!$result) {
                Logger::dayLog('remitrongbao', 'CRemit/addNotify', 'ClientNotify/saveData', $oClientNotify->errors);
                return false;
            }
        }
        return true;
