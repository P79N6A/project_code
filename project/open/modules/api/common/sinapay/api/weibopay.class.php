 ...
 * Date: 16/4/12
 * Time: 16:46
 * demo只是提供�?��接口对接编写的�?路，具体接口对接商户�?��以自身项目的实际情况来进行接口代码的编写�?
 */
...
//				self::write_log("MD5计算得出签名值：".$signMsg);
	...
	function curlPost($url, $data,$_input_charset) {
		...
		$timeLog->save('sinaapi', [$status, $data, $content]);
		self::write_log("请求新浪网关返回内容:".mb_convert_encoding($content,"UTF-8"));
	...
	function write_log($msg) {
		if(sinapay_debug_status){
			$filepath = \Yii::$app->basePath . '/log/weibopay/' . date ( "Ymd" ) . '.log';
