...
//----------------------------------------------------------------------
        ...
        Logger::dayLog('sjmh','SNotify/requeryNotify','数据：',$data,'加密数据：',$dataen);
        ...
        if(!$reason){
            $reason="未知错误";
            Logger::dayLog('sjmh/SNotify', 'doNotifyError', '通知失败：',$reason);
        }

        //4 保存状态
        $result = $oSjmhNotify->saveOneNotifyStatus($oNotify, $nextStatus, $reason);
        if (!$result) {
            Logger::dayLog('sjmh/SNotify', 'doNotify', $result->errors);
            return FALSE;
        }
        Logger::dayLog('sjmh/SNotify', 'doNotify', '通知成功请求id：',ArrayHelper::getValue($oNotify,'request_id'));
        return true;
    }
