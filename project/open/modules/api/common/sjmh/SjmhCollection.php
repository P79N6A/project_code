...
class SjmhCollection{
    ...
    #抓取返回错误码，处理方式为  稍后重试
    private $error_code = array(137,2007);
    ...
    public function oneDepositDate($value){
        ...
        //判断抓的数据状态码
        if($code != 0){
            if(in_array($code,$this->error_code)){
        ...
        if($userInfo){
            ...
            if(is_null($data['name']) || $data['name'] == 'null'){
                $data['name']='';
            }
            $data['mobile'] = ArrayHelper::getValue($userInfo,'mobile','');
            if(is_null($data['mobile']) || $data['mobile'] == 'null'){
                $data['mobile']='';
            }
            $data['idcard'] = ArrayHelper::getValue($userInfo,'idcard','');;
            if(is_null($data['idcard']) || $data['idcard'] == 'null'){
                $data['idcard']='';
            }
        }
        //保持抓取结果获取路径地址
        ...
        $result = $oGatherResult->saveData($gatherData);
     ...
     *  从抓取的数据中 获取一些基本的用户信息
     *  $data 抓取到的 数据
     *  $type SOURCE_SHE_BAO 社保 / SOURCE_CHSI 学信 / SOURCE_GJJ公积金 / SOURCE_JD京东
     * */
    public function getUserInfo($source,$data){
        ...
        if($source == $this->oSjmhDockingApi->gStatus('SOURCE_CHSI')){
            $returnData['name'] = ArrayHelper::getValue($data,'data.task_data.school_info.0.realname','');
            $returnData['idcard'] = ArrayHelper::getValue($data,'data.task_data.school_info.0.card_id','');
            $returnData['mobile'] = "";
        ...
        }elseif($source == $this->oSjmhDockingApi->gStatus('SOURCE_JD')){
            $returnData['name'] = ArrayHelper::getValue($data,'data.task_data.base_info.real_name','');
            $returnData['mobile'] = ArrayHelper::getValue($data,'data.task_data.base_info.mobile','');
            $returnData['idcard'] = ArrayHelper::getValue($data,'data.task_data.base_info.identity_code','');
        }elseif($source == $this->oSjmhDockingApi->gStatus('SOURCE_TB')){
            $returnData['name'] = ArrayHelper::getValue($data,'data.task_data.base_info.real_name','');
            $returnData['mobile'] = ArrayHelper::getValue($data,'data.task_data.base_info.mobile','');
            $returnData['idcard'] = ArrayHelper::getValue($data,'data.task_data.base_info.identity_code','');
        }elseif($source == $this->oSjmhDockingApi->gStatus('SOURCE_WY')){
            $returnData['name'] = ArrayHelper::getValue($data,'data.task_data.base_info.real_name','');
            $returnData['mobile'] = ArrayHelper::getValue($data,'data.task_data.base_info.mobile','');
            $returnData['idcard'] = ArrayHelper::getValue($data,'data.task_data.base_info.identity_code','');
        }else{
    ...
    public function getJsonData($taskId,$type){
        if(empty($taskId)){
            logger::dayLog('sjmh/collection','','task_id(任务id)不能为空',$taskId);
            return false;
        }
        $config = $this->config;
