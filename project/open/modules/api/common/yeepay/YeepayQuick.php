...
class YeepayQuick extends \yeepayMPay{
	public function __construct($env){
		$configPath = __DIR__."/config.{$env}/quick.php";
	...
	public function payRequest($postData){
		...
		try{
			$data = [
				...
				'paytypes'	=> (string)$postData['paytypes'],		//支付方式      string  格式：1|2|3|41- 借记卡支付；2- 信用卡支付；3- 手机充值卡支付；4- 游戏点卡支付注：'该参数若不传此参数，则默认选择运营后台为该商户开通的支付方式。
				'cardno'	=> (string)$postData['cardno'],			//银行卡序列号   在进行网页支付请求的时候，如果传此参数会把银行卡号直接在银行信息界面显示卡号，注意：P2P商户此参数须必填
				'idcardtype'=> (string)$postData['idcardtype'],	//证件类型      01：身份证，注意：证件类型和证件号必须同时为空或者同时不为空
