...
class YeepayTzt extends \yeepayMPay {
    public function __construct($env, $aid=0) {
        // 获取配置文件
        $config = $this->getConfig($env, $aid);
     ...
     * @return   []
     */
    private function getConfig($env, $aid) {
        if ($aid == 4) {
            // 花生米富使用独立商编
            $cfg = 'tzt4.php';
        } else {
        ...
        $configPath = __DIR__ . "/config.{$env}/{$cfg}";
    ...
    public function invokebindbankcard($postData) {
        ...
        $data = array(
            ...
            'idcardtype' => (string) $postData['idcardtype'], //证件类型√string固定值:01
            'idcardno' => (string) $postData['idcardno'], //证件号√string
            'username' => (string) $postData['username'], //持卡人姓名√string
            'phone' => (string) $postData['phone'], //银行预留手机号√string
            'registerphone' => (string) $postData['registerphone'], //用户注册手机号string  用户在商户的系统注册的手机号
            'registerdate' => (string) $postData['registerdate'], //用户注册日期string用户在商户的系统注册的日期，格式：yyyy-mm-dd hh:mm:ss精确到秒
            'registerip' => (string) $postData['registerip'], // 用户注册ipstring
            'registeridcardtype' => (string) $postData['registeridcardtype'], //用户注册证件类型string固定值:01
            'registeridcardno' => (string) $postData['registeridcardno'], //用户注册证件号 string
            'registercontact' => (string) $postData['registercontact'], //用户注册联系方式string手机号
            'os' => (string) $postData['os'], //用户使用的操作系统
            'imei' => (string) $postData['imei'], //设备唯一标识
            'userip' => (string) $postData['userip'], //用户请求ip√string用户支付时使用的网络终端IP
            'ua' => (string) $postData['ua'], //用户使用的浏览器信息
        );
        //print_r($data);exit;
        try {
