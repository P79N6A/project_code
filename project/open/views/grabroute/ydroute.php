...
<div class="selfmess">
	<div class="selftximg">
		...
		<?php if ($is_smscodejldx): ?>
        <div class="dbk_inpL">
			<label>验证码</label>
            <input id="validatecode"  style="font-size: 14px; padding: 5px;width: 100px;" placeholder="填写验证码" type="text" value="">
			<div style="position:inherit; font-size:14px; padding:6px 0 15px;"><b style="color:red;">&nbsp;*&nbsp;</b>请用本机发送CXXD至10001获取查询详单的验证码</div>
        </div>
		<?php endif; ?>
	...
	<input type="hidden" id='bizno' name="bizno" value="<?=isset($bizno)?$bizno:'';?>">
			...
			$.ajax({
				...
				data : {
					<?php if ($is_imgcode): ?>
						imgcode : imgcodes,
					<?php endif; ?>
					<?php if ($is_smscode || $is_smscodejldx): ?>
				...
				success  : function(data){
					if(data.res_code){//失败
						if(data.res_code == 40000 || data.res_code == 40003 || data.res_code == 60000){//账密错误
