...
class Bfxy181Controller extends ApiController {
    ...
    public function actionGetsmscode() {
        ...
        //3 发送短信
        $oCBfXY = new CBfXY181();
        $sendRes = $oCBfXY->sendSignSms($oBaofoo);
        if ($sendRes['res_code']) {
            return json_encode($sendRes,JSON_UNESCAPED_UNICODE);
        }
        return $this->showMessage(0, [
            'isbind' => false,
            'nexturl' => Yii::$app->request->hostInfo . '/bfxy181/paycomfirm',
        ]);
    }
    ...
    public function actionPaycomfirm() {
        ...
        //3 校验短信验证码
        $oCBfXY = new CBfXY181();
        $checkRes = $oCBfXY->checkSignSms($oBaofoo,$validatecode);
    ...
    public function actionBackpay($channelId = '') {
        ...
        //2 解析数据保存入库并通知业务端
        $resData = (new CBfXY181())->checkPayRes($dataContent,$channelId);
        if($resData['res_code']){
