...
class CBfXY {
    ...
    public function createOrder($oPayorder) {
        ...
        $this->channelId = $oPayorder->channel_id;
        $this->orderId = $oPayorder->id;
        //2. 保存签约信息
        $oBfSign = new BfSign();
        if (!$oBfSign->addSignInfo($oPayorder->attributes)) {
            Logger::dayLog('bfxy', 'createXYOrder: 创建协议支付子订单失败,主订单id为:'.$this->orderId.',失败原因:'.$oBfSign->errinfo);
            return StdError::returnStdError($this->channelId,"0012");
        }
        $bfOrderData = $oPayorder->attributes;
        $bfOrderData['bind_id'] = $oBfSign->id;
        $bfOrderData['cli_identityid'] = $oBfSign->cli_identityid;
        //3. 保存子订单
        $oBfXY = new BfXYOrder();
        if (!$oBfXY->addBfxyOrder($bfOrderData)) {
            Logger::dayLog('bfxy', 'createXYOrder: 创建协议支付子订单失败,主订单id为:'.$this->orderId.',失败原因:'.$oBfXY->errinfo);
            return StdError::returnStdError($this->channelId,"0012");
        }
        //4. 返回跳转Url地址
        $urlData = $oBfXY->getPayUrls();
     ...
     * @return array res_code=>返回码 res_data=>返回信息
     */
    public function sendSignSms($payOrderId,$channelId,$bindId) {
        // 获取配置文件
        ...
        if(!$oSignInfo){
            Logger::dayLog('bfxy', 'sendSignSms_Fail: 获取用户银行卡信息失败,主订单id为:'.$this->orderId);
     ...
     * @return array res_code=>返回码 res_data=>返回信息
     */
    public function checkSignSms($payOrderId,$channelId,$validatecode,$bindId) {
        // 获取配置文件
        $this->channelId = $channelId;
        $this->orderId = $payOrderId;
        $this->config = $this->getCfg();
        if (!$payOrderId || !$channelId || !$validatecode || !$bindId || !$this->config){
            return StdError::returnStdError("1630001","参数错误");
        }
        // 获取签约信息
        $oBfSign = new BfSign();
