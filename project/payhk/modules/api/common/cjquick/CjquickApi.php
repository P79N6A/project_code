...
use app\modules\api\common\cjquick\Util;
use Yii;
/**
 * 畅捷通支付类
 */
class CjquickApi {
    ...
    public function pay($data){
        $postdata = [
            ...
            'PayVersion' => $this->config['PayVersion'], //快捷版本0标准版 1升级版
        ];
    ...
    public function queryOrder($data){
        ...
        Logger::dayLog('cjquick/queryOrder', "请求信息：", $postdata);
        $result = $this->sendRequest($postdata);
        Logger::dayLog('cjquick/queryOrder', "请求结果：", $result);
        return $result;
    }
    /**
     * Undocumented function
    ...
    private function sendRequest($postdata){
        ...
        if($result){          
            $sign = $result['Sign'];
             ...
             if(!$res){
                 Logger::dayLog("cjquick/errorSign","rsaVerify验签失败", $url, $result);
                 return false;
             }
    ...
    public function verify($postdata){
        ...
        if(!$res){
            Logger::dayLog("cjquick/cjback","rsaVerify验签失败",$postdata);
            return false;
        }
        return $res;
    }
