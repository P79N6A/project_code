...
class FraudmetrixApi{
	private $config;
	private $type_reg_sign; // 判断是注册还是登录
	 ...
	 * 
	 */
	public function riskloan($type, $account_name, $account_mobile, $id_number, $seq_id, $ip_address, $token_id, $ext_school, $ext_diploma, $ext_start_year, $card_number, $pay_amount, $event_occur_time, $ext_birth_year, $organization, $ext_position, $black_box,$xhh_apps)
	{	 
		//同盾分配的合作方标示
		...
		//同盾分配的API秘钥
		if(in_array($xhh_apps, $apps)){
			$secret_key = $this->config['fraudmetrix_secret_key_'.$xhh_apps];
		}else{
		...
		if($type == 1){//借款
			if(in_array($xhh_apps, $apps)){
				$event_id = $this->config['fraudmetrix_loan_event_id_'.$xhh_apps];
			}else{
			...
			if(in_array($xhh_apps, $apps)){
				$event_id = $this->config['fraudmetrix_register_event_id_'.$xhh_apps];
			}else{
		...
		//返回设备信息参数 eg:device,geoip 描述：device，则返回设备指纹device_info信息；包含geoip，则返回用户传入的IP地理位置解析geoip_info信息；包含attribution，则返回身份证和手机号归属地信息；包含credit_score，则返回结果包含同盾智信分包含application_id，借款事件申请编号,若用户传入则返回结果包含该信息。 
        $resp_detail_type = 'device,geoip,attribution,application_id';
		...
		$data = array(
				...
				'black_box' => $black_box,
				'resp_detail_type' => $resp_detail_type
		);
		Logger::dayLog("risk/params", "请求第三方参数：", json_encode($data));
	 ...
	 * @return int
	 */
	public function regAndLog($params)
	{
		if (empty($params)){
			return false;
		}
		//判断是登录还是注册  1是注册  2 登录
		$this->type_reg_sign = ArrayHelper::getValue($params, 'type',1);
		//配置参数
		$must_params = $this->mustParams();
		$params = array_merge($params, $must_params);

		//1.请求第三方
		Logger::dayLog("hsmfweb/params", "请求第三方参数：", json_encode($params));
		$result = $this->invoke_fraud_api($params, ArrayHelper::getValue($this->config, 'apiUrl'));
		//2.保存文件
		...
		if(!empty($result) && !empty($result['seq_id'])){
			sleep(2);
			//合作方代码，在用户接入时由同盾分配，用于校验
			$hitruledetail_partner_code = $this->config['hsmf_partner_code'];
			//合作方密钥
			$hitruledetail_partner_key = $this->config['hsmf_partner_key'];
			//事件ID
			$sequence_id = $result['seq_id'];
			$hitruledetail_api_url = $this->config['hsmf_hit_api_url'];
			$data = [
				'partner_code'   	=> $hitruledetail_partner_code,
				'partner_key' 		=> $hitruledetail_partner_key,
				'sequence_id' 		=> $sequence_id,
			];
			Logger::dayLog("hsmfweb/params", "请求第三方参数hit：", json_encode($data));
			$ret = $this->curlGet($hitruledetail_api_url, $data);
			$detailurl = str_replace(".json", "_detail.json", $url);
			$this->saveDetail($detailurl, $ret);
			$result['rules'] =json_decode($ret, true);
			return $result;
		}
		return $result;
	}
	 ...
	 * @return array
	 */
	private function mustParams()
	{
		$secret_key = $this->type_reg_sign == 1 ? ArrayHelper::getValue($this->config, 'hsmf_reg_event_id') : ArrayHelper::getValue($this->config, 'hsmf_log_event_id');
		return  [
			//'apiUrl'                => ArrayHelper::getValue($this->config, 'apiUrl'), //String	API请求地址
			'partner_code'          => ArrayHelper::getValue($this->config, 'hsmf_partner_code'), //String	合作方标识
			'secret_key'            => ArrayHelper::getValue($this->config, 'hsmf_secret_key'), //String	API请求秘钥
			'event_id'				=> $secret_key,
		];
	}
