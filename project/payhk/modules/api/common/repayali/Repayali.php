...
namespace app\modules\api\common\repayali;
...
class Repayali
{
    ...
    public function __construct($cfg=151)
    {
        $oConfig = new Config();
        try {
     ...
     * @return array
     */
    public function createOrder($data_set)
    {
        Logger::dayLog("repayali/createOrder", "接收参数：", $data_set);
        //1. 格式数据
        $data_set = $this->sendFormat($data_set);
        //$data_set = ["merchantOutOrderNo"=>'1268754'.time(), "orderMoney" => 0.01, 'payorder_id'=>'1222'];
        Logger::dayLog("repayali/createOrder", "格式参数：", json_encode($data_set));
        ...
        //3.记录数据
        $ret = $this->saveOrder($data_set, PayAlipayOrder::TYPE_SIX);
        if ($ret !== '0000'){
        ...
        $aliPayURL = ArrayHelper::getValue($this->__config, 'request_url') . $params;
        Logger::dayLog("repayali/createOrder", "返回的url：", $aliPayURL);
        return $this->successMsg(['msg'=>$aliPayURL]);
    }
    ...
    public function queryOrder($order_id)
    {
        ...
        $key = $this->getKey($order_id);
        //$key = "321ef04116cf4140a713bbbc19c72883";
        if (empty($key)){
            return $this->returnMsg('0001');
        }
        $pay_merid = $this->getMerid($order_id);
        //$pay_merid = '101100117';
        if (empty($pay_merid)){
            return $this->returnMsg('0001');
        }
        ...
        Logger::dayLog("repayali/queryOrder", "返回数据：", json_encode($result_data));
        //如果订单号不存在
    ...
    public function getMerid($order_id)
    {
        ...
        $merid_data = [
            ...
            '151' => ArrayHelper::getValue($this->__config, 'merid'),
            '153' => ArrayHelper::getValue($this->__config, 'merid'),
            '154' => ArrayHelper::getValue($this->__config, 'merid'),
            '156' => ArrayHelper::getValue($this->__config, 'merid'),
        ];
    ...
    public function getKey($order_id)
    {
        ...
        $merid_data = [
            ...
            '151'       => ArrayHelper::getValue($this->__config, 'key'),
            '153'       => ArrayHelper::getValue($this->__config, 'key'),
            '154'       => ArrayHelper::getValue($this->__config, 'key'),
            '156'       => ArrayHelper::getValue($this->__config, 'key'),
        ];
