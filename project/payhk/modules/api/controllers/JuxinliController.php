...
class JuxinliController extends ApiController {
	...
	public function actionPostrequest() {
		...
		$data['source'] = (new JxlRequestModel)->getAutoSource($data['phone'],$website);
	...
	public function actionQueryall() {
		...
		//2 查找文件
		$oJxlStat = new JxlStat();
		$appId = ArrayHelper::getValue($this->reqData,'appId','0');
		$data = $oJxlStat->getDataByPhoneAll($phone,$website,$appId);
		if (empty($data)) {
			return $this->resp(8134, "数据为空");
		}
		//4 返回结果
	...
	/**
	 * 获取运营商的所有数据 按照实际降序排列  根据number返回对象数据
	 * @return array
	 */
	public function actionQuerynumber() {
		...
		$count = count($data);
		if(intval($number)>$count || intval($number)<=0){
			return $this->resp(8134, "number参数有误，总条数".$count);
		}
		$result = json_decode(ArrayHelper::getValue($data,$number-1,[]),true);
		if (empty($result)) {
			return $this->resp(8134, "数据为空");
		}

		$result['total'] = $count;
		$result['now_number'] = $number;
		//4 返回结果
	 ...
	 * 危险操作
	 */
	private function actionAccessReportToken() {
