...
class WxqueryController extends BaseController
{
    private $env;
    ...
    public function runQuerys()
    {
        ...
        $data_set = $oPayAlipayOrder->getOrderWxData($start_time, $end_time, 200);
        if (!empty($data_set)){
            ...
            foreach($data_set as $value) {
                $oRepayment = new Repaywx(ArrayHelper::getValue($value, 'channel_id'));
