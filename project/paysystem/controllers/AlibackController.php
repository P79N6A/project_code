...
class AlibackController extends ApiController
{
    ...
    public function actionNotify()
    {
        ...
        Logger::dayLog('repayali/wxback', '接收参数：',$poststr);
        if (empty($poststr)){
            Logger::dayLog('repayali/wxback', '错误：', '参数为空');
        ...
        if ($new_sign != $sign){
            Logger::dayLog('repayali/wxback', '错误：', json_encode($params_data)."--sign验证不正确");
        ...
        if (!$verify_state){
            Logger::dayLog('repayali/wxback', '错误：', $verify_state);
            return $oRepayment->returnMsg('0001', true);
        }
        //更新数据
