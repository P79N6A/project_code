...
class BindBank extends \app\models\BaseModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['identityid', 'cardno', 'bankname', 'validate', 'cvv2'], 'string', 'max' => 50],
    ...
    /**
     * @desc 绑卡成功之后插入绑卡记录
     * @param obj $olabindbank
     * @return bool
     **/
    public function succBindBank($olabindbank) {
        //1 获取绑定信息
        $oBind = $this->getBindBankInfo($olabindbank->aid, $olabindbank->identityid, $olabindbank->cardno, $olabindbank->channel_id);
        $result = true;
        if (empty($oBind)) {
            $bindData = [
                'channel_id' => $olabindbank->channel_id,
                'aid' => $olabindbank->aid,
                'identityid' => $olabindbank->identityid,
                'idcard' => $olabindbank->idcard,
                'name' => $olabindbank->name,
                'cardno' => $olabindbank->cardno,
                'card_type' => 1, // 连连仅支付借记卡
                'status'=>self::STATUS_BINDOK, //绑卡成功
                'phone' => $olabindbank->phone,
                'bankname' => $olabindbank->bankname,
                'userip' => $olabindbank->userip,
            ];
            $result = $this->saveOrder($bindData);
        }
        return $result;
    }
    /**
     * 更新绑卡信息
     */
    public function updateBindBank($up_data,$where){
        $result = static::updateAll($up_data,$where);
        return $result;
    }
}
