 ...
 *
 */
class CjtOrder extends \app\models\BasePay {
    /**
     * @inheritdoc
     */
     ...
     * @return   str
     */
    public function getAOrderId($orderid) {
        return substr($orderid,2);
    }
    ...
    public function saveOrder($oPayorder) {
        //1 数据验证
        if (!is_object($oPayorder) || empty($oPayorder)) {
        ...
        $cli_orderid = $this->getAOrderId($oPayorder->orderid);
     ...
     * @return bool
     */
    public function saveStatus($status,$res_code,$res_msg,$other_orderid) {
        if ($other_orderid) {
     ...
     * @return []
     */
    public function getPayUrls($pay_controller='baofoo',$pay_type='') {
