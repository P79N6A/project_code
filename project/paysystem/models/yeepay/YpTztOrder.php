 ...
 * 易宝投资通
 */
class YpTztOrder extends \app\models\BasePay {
     ...
     * @return []
     */
    public function getPayUrls($pay_controller='yeepay', $pay_type='') {
    ...
    public function getAbnorList($start_time,$end_time){
		...
		$dataList = self::find()->where($where)->limit(200)->all();
		if (!$dataList) {
			return null;
		}
		return $dataList;
