...
namespace app\models\yimadai;

use app\models\Payorder;
use yii\helpers\ArrayHelper;

/**
 * This is the model class for table "pay_ymdxy_order".
 *
 * @property integer $id
 ...
 */
class YmdxyOrder extends \app\models\BasePay {

    /**
     * @inheritdoc
     */
    public static function tableName() {
        return 'pay_ymdxy_order';
    }
    ...
    public function getByOrderid($orderid) {
        ...
        //2. 获取订单数据
        $data= self::find()->where(['cli_orderid'=>$orderid])->one();
        if(empty($data)){
            return 0;
        }
        return $data->id;
     ...
     * @return []
     */
    public function getPayUrls($pay_controller = 'ymdxy', $pay_type = '') {
     ...
     * @param string $token_no   签约号
     * @param string $res   错误信息
     * @return bool
     */
    public function saveStatus($status, $other_orderid='',$token_no='',$res='') {
        if (!empty($other_orderid)) {
            $this->other_orderid = (string) $other_orderid;
        }
        if (!empty($token_no)) {
            $this->token_no = (string) $token_no;
        }
