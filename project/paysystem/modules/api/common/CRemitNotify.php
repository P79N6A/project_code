...
class CRemitNotify {
	...
	public function __construct() {
		$this->oClientNotify = new RemitNotify;
		$this->logname = 'remitnotify';
	}
	/**
	 * 一般是每几分钟执行
	 */
	...
	protected function doNotify($oNotify) {
		...
		$data = [
			...
			'settle_amount' => $oNotify['settle_amount'],
			'remit_status' => $oNotify['remit_status'],
			'rsp_status' => $oNotify['rsp_status'],
			//'rsp_status_text' => $oNotify['tip'],
			'tip' => $oNotify['tip'],
        ];
	 	$dataen = $this -> encryptData($oNotify['aid'], $data);
		$response = $this->curlPost($oNotify['callbackurl'], $dataen);
		if ($response == 'SUCCESS') {
