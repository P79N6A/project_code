...
if (!function_exists( 'hex2bin')) {
 ...
 class BFRSA{
    ...
    */
    ...
    /********** 协议支付使用公钥加密 私钥解密补充 **********/
    // 公钥加密
    function encryptByCERFile($Data){
        ...
        while ($EncryptSubStarLen < $totalLen){
            openssl_public_encrypt(substr($BASE64EN_DATA, $EncryptSubStarLen, $blockSize), $EncryptTempData, $this->public_key);
    ...
    // 私钥解密
    function decryptByPFXFile($Data){
        $DecryptRsult="";
        $blockSize=256;//分段长度
        ...
        $DecryptTempData="";
        while ($EncryptSubStarLen < $totalLen) {
            openssl_private_decrypt(hex2bin(substr($Data, $EncryptSubStarLen, $blockSize)), $DecryptTempData, $this->private_key);
