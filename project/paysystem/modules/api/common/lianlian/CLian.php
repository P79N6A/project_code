...
class CLian {
    ...
    public function signApply($oLianOrder) {
        ...
        $res = $this->getApi($oLianOrder['channel_id'])->signApply($payData);
        if ($res['res_code'] > 0) {
    ...
    public function apply($oLianOrder) {
        ...
        $res = $this->getApi($oLianOrder['channel_id'])->apply($payData);
    ...
    public function pay($oLianOrder) {
        ...
        //2. 转成元(目前是金额的单位是分)
        $amount = $oLianOrder->amount / 100;
        $notify_url = Yii::$app->request->hostInfo . '/lianpay/backpay/';
        // // 将回调改到测试开发平台
        // $notify_url = 'http://182.92.80.211:8091/lianpay/backpay/';
        ...
        $res = $this->getApi($oLianOrder['channel_id'])->pay($payData);
    ...
    private function getBindBank($data) {
        ...
        if (empty($oBind)) {
            ...
            if (!$result) {
                Logger::dayLog('lian', "clian/getBindBank", $oBind->attributes, $oBind->errinfo);
            }
    ...
    public function verifyJson($json_data, $channel_id) {
        ...
        return $this->getApi($channel_id)->verify($data);
    }
    /**
     * 用于绑卡回调加解密
     * @param  int $lianid
