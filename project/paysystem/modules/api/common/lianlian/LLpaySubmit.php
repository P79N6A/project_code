 ...
 */
if (!defined('LIANLIAN')) {
    define('LIANLIAN', true);
    require __DIR__ . "/llpay_core.function.php";
    require __DIR__ . "/llpay_md5.function.php";
    require __DIR__ . "/llpay_rsa.function.php";
}
class LLpaySubmit {
    ...
    function getSignVeryfy($para_temp, $sign) {
        ...
        switch (strtoupper(trim($this->llpay_config['sign_type']))) {
            case "MD5" :
                ...
                break;
            case "RSA" :
                $isSgin = Rsaverify($prestr, $sign,$this->llpay_config['LIANLIAN_PUBLICK_KEY']);
                break;
            default :
                $isSgin = false;
        }
     ...
     * @return 提交表单HTML文本
     */
    public function buildRequestFormCode($url, $req_data,$post='POST') {
        $sHtml = "<form id='llpaysubmit' style='display:none;' name='llpaysubmit' action='{$url}' method='{$post}'>";
