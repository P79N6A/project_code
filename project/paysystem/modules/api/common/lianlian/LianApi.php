...
class LianApi {
    ...
    public function notifyverify($data) {
        ...
        $sign1 = $data['sign'];
        unset($data['sign']);
        $result = $this->oLianService->getSignVeryfy($data,$sign1);
        return $result;
    }
    /**
     * 生成要请求给连连支付的参数数组
     ...
     * @return [res_code, res_data]
     */
    public function authpay($payData) {
        $data = [
            'version' => $this->config['version'],
            'oid_partner' => $this->config['oid_partner'],
            'app_request' =>$this->config['app_request'],
     ...
     *
     **/
     public function authquery($payData) {
        $data = [
            'oid_partner' => $this->config['oid_partner'],
            'sign_type' => $this->config['sign_type'],
            'no_order'=> $payData['no_order'],
            'dt_order'=>$payData['dt_order'],
            'query_version' => $this->config['version']
        ];
        $signData = $this->buildSign($data);
        $url = $this->config['query_pay'];
        $res = $this->curlPost($url, $signData);
        $res = json_decode($res, true);
        // 返回结果
        Logger::dayLog('lian', 'lianapi/authquery', $url, $signData, $res);
        $response = $this->parseResult($res);
        return $res;
    }
    ...
    public function acctquery(){
        $data = [
            'oid_partner' => $this->config['oid_partner'],
            'sign_type' => $this->config['sign_type'],
            'api__version' => $this->config['version']
        ];
        $signData = $this->buildSign($data);
        $url = $this->config['url_acct_query'];
        $res = $this->curlPost($url, $signData);
        $res = json_decode($res, true);
        // 返回结果
        Logger::dayLog('lian', 'lianapi/acctquery', $url, $signData, $res);
        $ret_code = ArrayHelper::getValue($res, 'ret_code');
        $ret_msg = ArrayHelper::getValue($res, 'ret_msg', '未知错误');
        if ($ret_code == '0000') {
            $result = $this->notifyverify($res);
            if (!$result) {
                Logger::dayLog('lian', 'lianapi/acctquery','验签失败',$verifyResult);
                return ['res_code' => 'sign_error', 'res_data' => "签名不正确"];
            }
            return ['res_code' => 0, 'res_data' => $res['amt_balance']];
        } else {
            return ['res_code' => $ret_code, 'res_data' => $ret_msg];
        }
        return $res;
