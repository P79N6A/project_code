...
class BfremitController extends AdminController {
    ...
    public function actionIndex() {
        ...
        $pages = new Pagination([
            'totalCount' => BfRemit::find()->where($where)->count(),
            'pageSize'   => '20'
        ]);
        $res   = BfRemit::find()->where($where)->offset($pages->offset)->limit($pages->limit)->orderBy('id desc')->all();
    ...
    public function actionUpdate(){
        if ($this->isPost()) {
            $post = $this->post();
            $ipInfo = BfRemit::findOne($post['id']);
