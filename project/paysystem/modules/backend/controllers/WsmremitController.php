...
class WsmremitController extends AdminController {
    ...
    public function actionIndex() {
        ...
        $pages = new Pagination([
            'totalCount' => WsmRemit::find()->where($where)->count(),
            'pageSize'   => '20'
        ]);
        $res   = WsmRemit::find()->where($where)->offset($pages->offset)->limit($pages->limit)->orderBy('id desc')->all();
    ...
    public function actionUpdate(){
        if ($this->isPost()) {
            $post = $this->post();
            $ipInfo = WsmRemit::findOne($post['id']);
