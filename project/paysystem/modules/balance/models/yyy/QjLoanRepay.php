 ...
 */
class QjLoanRepay extends YyyBase
{
    /**
     * @inheritdoc
     */
    ...
    {
        if (empty($loan_id)){
            $query = QjLoanRepay::find();
            $query->select([
                QjLoanRepay::tableName().'.*',
//                Repay_coupon_use::tableName().'.*',
                'DATE_FORMAT('.QjLoanRepay::tableName().'.last_modify_time,\'%Y-%m-%d\') as lastData',
