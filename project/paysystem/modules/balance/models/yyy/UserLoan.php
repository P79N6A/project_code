 ...
 */
class UserLoan extends YyyBase {
...
//                ];
     ...
     */
    public function getAllSum($filter_where)
    {

        /*settle_type  状态0  代表该笔借款没有做过续期
         * settle_type  状态3  status=8   代表该笔借款在本次续期中 已经结清 全部还款
         * */
        $start = ArrayHelper::getValue($filter_where,'start_time').' 00:00:00';
        $end_time = ArrayHelper::getValue($filter_where,'end_time').' 23:59:59';
        $fund = ArrayHelper::getValue($filter_where,'fund');
        $platform = ArrayHelper::getValue($filter_where,'wayOfPayment');

        $where_config = "";
        //体外还款
        if (!empty($platform) && $platform == 1){
            $where_config.= " AND yi_loan_repay.platform not in ('26', '27')";
        }

        //体内还款
        if (!empty($platform) && $platform == 10){
            $where_config.= " AND yi_loan_repay.platform in ('26', '27')";
        }
        //资金方
        if (!empty($fund)){
            $where_config.= " AND yi_user_remit_list.fund={$fund}";
        }

        $sql = " SELECT 
                        DATE(datetimes) AS datetimes,
                        count(1) AS nums,
                        sum( money ) AS money,
                        sum( fee ) AS fee,
                ...
                from( 
                      SELECT 
                        DATE(yi_user_loan.repay_time) AS datetimes, 
                        (yi_user_loan.amount) AS money, 
                        (yi_user_loan.interest_fee) AS fee, 
                        (yi_user_loan.coupon_amount) AS coupon,
                        (yi_user_loan.like_amount) AS likes, 
                        days, 
                        yi_user_loan.loan_id as loan_id, 
                        yi_loan_repay.platform as platform,
                        (yi_loan_repay.actual_money) as actual_money,
                        yi_user_remit_list.fund as funds 
                      FROM `yi_user_loan` 
                      LEFT  JOIN `yi_loan_repay` ON yi_loan_repay.loan_id = yi_user_loan.loan_id 
                      LEFT  JOIN `yi_user_remit_list` ON yi_user_remit_list.loan_id = yi_user_loan.loan_id
                      WHERE ( 
                                ( yi_user_loan.repay_time < yi_user_loan.end_date ) AND 
                                ( `yi_user_loan`.`create_time` >= '2018-01-01' ) ) AND 
                                ( `yi_user_loan`.`status` = '8' ) AND 
                                ( `yi_user_loan`.`settle_type` in('0','3'))  and 
                                (yi_user_loan.repay_time >= '".$start."') and 
                                (yi_user_loan.repay_time <= '".$end_time."')
                                {$where_config} 
                      GROUP BY yi_user_loan.loan_id 
                      ORDER BY `datetimes` DESC 
                ) a 
                GROUP BY `datetimes`, `days`, `platform`) c";
        //echo $sql;
    ...
    private function repayWhere($filter_where)
    {
        ...
        $result = self::find()
                ->select(
                    [
                        'DATE(yi_user_loan.repay_time) AS datetimes,count(1) AS nums',
                        'sum( amount ) AS money',
                        'sum( interest_fee ) AS fee',
                        'sum(coupon_amount ) AS coupon',
                        'sum( like_amount ) AS likes, days, yi_user_loan.loan_id'
                    ])
        ...
        if (!empty($filter_where['wayOfPayment'])){
            $result->andWhere(['yi_loan_repay.platform' => $filter_where['wayOfPayment']]);
            $platform = [26,27];
            if($filter_where['wayOfPayment'] ==1 ){
                $result->andWhere(['not in', 'yi_loan_repay.platform',$platform]);
            }else{
                $result->andWhere(['in', 'yi_loan_repay.platform',$platform]);
            }

        }
        if (!empty($filter_where['fund'])){
            //$result->andWhere(['fund' => $filter_where['fund']]);
            $result->andWhere(['=', 'yi_user_remit_list.fund', $filter_where['fund']]);
        }
        if (!empty($filter_where['start_time'])){
            $result->andWhere(['>=', 'yi_user_loan.repay_time', $filter_where['start_time']. ' 00:00:00']);
        }
        if (!empty($filter_where['end_time'])){
            $result->andWhere(['<=', 'yi_user_loan.repay_time', $filter_where['end_time']. ' 23:59:59']);
        }
        $result->andWhere('yi_user_loan.repay_time < yi_user_loan.end_date');
        $result->andWhere(['>=', 'yi_user_loan.create_time', '2018-01-01']);
        $result->andWhere(['yi_loan_repay.status'=>'8']);
        $result->andWhere(['yi_user_loan.settle_type'=>[0,3]]);
        return $result;
    }
    /**
     * 逾期条数
 ...
 */
    ...
    public function getAllDatas($pages, $filter_where)
    {
        ...
        $sql = " SELECT 
                    ...
                    funds,
                    sum(money) as tomoney,
                    count(1) AS nums,
                    sum(fee) as tofee,
                    sum(coupon) as tocoupon,
                    sum(likes) as tolikes,
                    datetimes,
                    platform,
                    days  
                from( 
                ...
                GROUP BY `datetimes`, `days`, `platform` LIMIT {$pages->offset}, {$pages->limit}";
        //echo $sql;
     ...
     * @return array|bool|\yii\db\ActiveRecord[]
     */
    public function getAllTotal($filter_where)
    {
        ...
        $sql = " SELECT COUNT(1) as count FROM( SELECT 
                    sum( actual_money) as actual_money, 
                    funds,
                    sum(money) as tomoney,
                    count(1) AS nums,
...
//        $result->andWhere(['settle_type'=>[0,3]]);
...
//            ->leftJoin(Repay_coupon_use::tableName(),Repay_coupon_use::tableName().'.loan_id='.self::tableName().'.loan_id')
    ...
    private function beforeWhere($condition)
    {
        ...
        $result = self::find()
            ...
            ->leftJoin("yi_user_remit_list","yi_user_remit_list.user_id=yi_user_loan.user_id")
            ->leftJoin("yi_user","yi_user.user_id=yi_user_loan.user_id");
        ...
        $result->andWhere(['=', 'yi_user_remit_list.remit_status',"SUCCESS"]);
        //业务类型
        if (!empty($condition['days'])){
            $result->andWhere(['=', 'yi_user_loan.days', $condition['days']]);
        }
        ...
        if (!empty($condition['capital_side'])){
            $result->andWhere(['=', 'yi_user_remit_list.fund', $condition['capital_side']]);
        }
        if (!empty($condition['start_time'])){
            $result->andWhere(['>=', 'yi_user_loan.end_date', $condition['start_time']. ' 00:00:00']);
        }
        ...
        $result->groupBy("yi_user_loan.days, yi_user_loan.end_date");
        return $result;
    }
