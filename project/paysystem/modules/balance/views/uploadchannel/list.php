            ...
            <table class="table table-bordered table-condensed table-hover table-striped dataTable no-footer">
                <thead>
                    <tr role="row">
                        ...
                        <th style="text-align: center;" width="20%">账单时间范围</th>
                ...
                <?php
                    if (!empty($getAllData)) {
                        ...
                        foreach($getAllData as $value) {
                            ...
                            ?>
                            <tr role="row">
                                <td style="text-align: center;" width="10%"><?=$num?></td>
                                <td style="text-align: center;" width="10%"><?=ArrayHelper::getValue($return_channel, ArrayHelper::getValue($value, 'channel_id'), 0)?></td>
                                <td style="text-align: center;" width="20%"><?=ArrayHelper::getValue($source, ArrayHelper::getValue($value, 'source', 0), '')?></td>
                                <td style="text-align: center;" width="30%"><?=ArrayHelper::getValue($value, 'start_time', '')?>至<?=ArrayHelper::getValue($value, 'end_time', '')?></td>
                                <td style="text-align: center;" width="20%"><?=ArrayHelper::getValue($value, 'create_time', '')?></td>
                               <!-- <td style="text-align: center;" width="20%"><a href="/balance/uploadchannel/dateilslist?id=<?/*=ArrayHelper::getValue($value, 'id')*/?>&return_channel=<?/*=ArrayHelper::getValue($value, 'channel_id')*/?>&source=<?/*=ArrayHelper::getValue($value, 'source')*/?>">详情</a></td>-->
                            </tr>
                            <?php
                        }
                    }else {
...
//跳转到上传页面
function jump_url(){
    location.href="/balance/uploadchannel/upbill";
}
</script>