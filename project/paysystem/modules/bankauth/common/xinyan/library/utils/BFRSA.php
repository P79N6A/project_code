<?php
namespace app\modules\bankauth\common\xinyan\library\utils;
...
class BFRSA
{
    ...
    function __construct($private_key_path, $public_key_path, $private_key_password, $Debug = FALSE)
    {
        ...
        Logger::dayLog('xinyan/key', 'is_key',empty($private_key) == true ? "私钥是否可用:不可用" : "私钥是否可用:可用");
        ...
        Logger::dayLog('xinyan/key', 'is_public_key',empty($this->public_key) == true ? "公钥是否可用:不可用" : "公钥是否可用:可用");
        if (!$Debug) {
            ob_end_clean();
        }
    }
