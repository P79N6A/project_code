...
if (!class_exists('Crypt_Rijndael')) {
    include __DIR__.'/Crypt_Rijndael.php';
}
    
if (!class_exists('Crypt_AES')) {
    include __DIR__.'/Crypt_AES.php';
}
if (!class_exists('Crypt_DES')) {
    include __DIR__.'/Crypt_DES.php';
}
if (!class_exists('Crypt_Hash')) {
    include __DIR__.'/Crypt_Hash.php';
}
    
if (!class_exists('Crypt_RSA')) {
    include __DIR__.'/Crypt_RSA.php';
}
    
if (!class_exists('Crypt_TripleDES')) {
    include __DIR__.'/Crypt_TripleDES.php';
}
    
if (!class_exists('Math_BigInteger')) {
    include __DIR__.'/Math_BigInteger.php';
}
...
class yeepayMPay
{
    ...
    // 访问地址
    private $API_Pay_Base_Url;
    private $API_Mobile_Pay_Base_Url;
    private $API_PC_Pay_Base_Url;
    private $API_Merchant_Base_Url;
    ...
    /**
     * @param $env dev|prod 可由子类设置
     */
    protected function setApiEnv($cfg = 'dev')
    {
        // 当配置了生成地址的环境变量时才是
        if ($cfg == 'dev') {
            //	测试环境请求基础地址
            $this->API_Pay_Base_Url = 'http://mobiletest.yeepay.com/testpayapi/api/';
            $this->API_Mobile_Pay_Base_Url = 'http://mobiletest.yeepay.com/paymobile/';
            $this->API_PC_Pay_Base_Url = 'http://mobiletest.yeepay.com/payweb/';
            $this->API_Merchant_Base_Url = 'http://mobiletest.yeepay.com/merchant/';
        } else {
            //	正式环境请求基础地址
            $this->API_Pay_Base_Url = 'https://ok.yeepay.com/payapi/api/';
            $this->API_Mobile_Pay_Base_Url = 'https://ok.yeepay.com/paymobile/';
    ...
    */
    ...
    protected function post($type, $method, $query)
    {
        ...
        if ($this->http_info['http_code'] != 200) {
            return ['code'=>$this->http_info['http_code'],'data'=>'__timeout'];
        }
    ...
    */
    ...
    protected function parseReturnData($data)
    {
        $return = json_decode($data, true);
        if (empty($return)) {
            //throw new yeepayMPayException('no response',2000);
            return null;
        }
// 		if(array_key_exists('error_code', $return))
//		for api : query/order
        if (array_key_exists('error_code', $return) && !array_key_exists('status', $return)) {
            return ['error_code'=>$return['error_code'], 'error_msg'=>$return['error_msg']];
//            throw new yeepayMPayException($return['error_msg'], $return['error_code']);
        }
        return $this->parseReturn($return['data'], $return['encryptkey']);
    }
    protected function parseReturn($data, $encryptkey)
    {
        ...
        $return = json_decode($return, true, 5, JSON_BIGINT_AS_STRING);
