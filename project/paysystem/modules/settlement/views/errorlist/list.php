    ...
    <div class="box ui-sortable-handle" style="position: relative; left: 0px; top: 0px;">
        ...
        <div class="body">
            ...
            <div style="margin-bottom: 10px">
                ...
                差错类型：<select name="error_types" style="margin-right: 10px;">
                        <option value="0">请选择</option>
                        <?php
                        foreach($errorTypes as $key=>$value) {
                            ?>
                            <option value="<?=$key?>" <?=(ArrayHelper::getValue($getData, 'error_types', 0)==$key) ? "selected=selected" : ""?>  ><?=$value?></option>
                            <?php
                        }
                        ?>
                    </select>
            ...
            <table class="table table-bordered table-condensed table-hover table-striped dataTable no-footer">
                ...
                <tr role="row">
                    ...
                    <td><?=ArrayHelper::getValue($value, 'settle_amount', '')?>/<?=ArrayHelper::getValue($value, 'amount', '')?>/元</td>
                    <td><?=ArrayHelper::getValue($value, 'settle_fee', '')?>/元</td>
                    <td><?=ArrayHelper::getValue($errorTypes, ArrayHelper::getValue($value, 'error_types', 0), '')?></td>
                    <td><?=date("Y-m-d", strtotime(ArrayHelper::getValue($value, 'bill_number', '')))?></td>
                    <td><?=ArrayHelper::getValue($value, 'create_time', '')?></td>
                    <td><a href="/settlement/errorlist/details?id=<?=ArrayHelper::getValue($value, 'id', '').'&'.$url_params?>">详情</a></td>
                </tr>
                <?php
                    }
                }
...
$(function(){
    ...
    $("#down_url").click(function(){
        location.href="/settlement/errorlist/down?"+url_params;
    });
})
</script>