...
<div class="selfmess">
	<div class="selftximg">
		...
		<div class="dbk_inpL">
        	<label>支付金额</label>
            <div class="input">￥<?=($oCjOrder->amount)/100?></div>
            <!--input placeholder="" type="text"-->
    	</div>
        <div style="height:20px; background:#e7edf0;"></div>
    	<div class="dbk_inpL">
        	<label>姓名</label>
            <div class="input"><?=\app\common\Func::strProtected($oBind->name)?></div>
    	</div>
    	<div class="dbk_inpL">
        	<label>身份证</label>
            <div class="input"><?=substr_replace($oBind->idcard,"****",4,10)?></div>
    	</div>
        <div class="dbk_inpL">
            <label>手机号</label>
            <div class="input"><?=substr_replace($oBind->phone,"****",3,4)?></div>
        </div>
        <div class="dbk_inpL">
            <label>银行卡</label>
            <div class="input"><?=substr_replace($oBind->cardno,"****",0,-4)?></div>
        </div>
		 <?php if($oCjOrder->bankcardtype == 2):?>
         <div class="dbk_inpL">
            <label>有效期</label>
            <input id="expiry_date" class="yzmwidth" maxlength="5" placeholder="填写有效期,如10/16" type="text">
        </div>
        <div class="dbk_inpL">
            <label>CVV2</label>
	...
	me.getsmscode = function(){
		$.post(
				"/cjpay/getsmscode",
				{
					...
					expiry_date:me.expiry_date.val(),
					cvv2:me.cvv2.val(),
					_csrf : me._csrf
				},
				function(data){
