...
class ApiSsReport(db.Model, Base):
    # 指定数据库
    __bind_key__ = 'xhh_antifraud'
    __tablename__ = 'api_ss_report'

    id = db.Column(db.BigInteger, primary_key=True)
    base_id = db.Column(db.BigInteger)
    score = db.Column(db.Integer) 
    rain_risk_reason = db.Column(db.Text)
    rain_score = db.Column(db.String)
    consume_fund_index = db.Column(db.String)
    indentity_risk_index = db.Column(db.String)
    social_stability_index = db.Column(db.String)
