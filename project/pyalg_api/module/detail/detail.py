...
class Detail(object):
    ...
    def _analysis(self):
        ...
        self.com_start_time = pd_detail.start_time[pd.notnull(pd_detail['start_time'])].min()
        self.com_end_time = pd_detail.start_time[pd.notnull(pd_detail['start_time'])].max()
    ...
    def fillnaStartTime(self, start_time):
        start_time = start_time[(-start_time.isin(['未知']))]
    ...
    def run(self):
        ...
        com_night_duration_p = 0 if com_duration == 0 else float(com_night_duration) / com_duration
        ...
        #关机时长
        dates = pd_detail.start_time[pd.notnull(pd_detail['start_time'])].sort_values(ascending=False)
        ...
        try:
            other_dict = {
                'retain_ratio': float('%.2f' % (int(same_phone_num) / int(tot_phone_num))) if int(
                    tot_phone_num) > 0 else '',
                'last_3mth_Oth_ratio': float('%.2f' % (int(last3_not_mobile_count) / int(last3_all))) if int(
                    last3_all) > 0 else '',
                'last_3mth_oth_incr': float(
                    '%.2f' % (int(last3_not_mobile_count) / int(last6_not_mobile_count))) if int(
                    last6_not_mobile_count) > 0 else '',
            }
        except Exception as e:
            logger.error("detail add other_data is fail: %s" % (e))
            other_dict = {
                'retain_ratio': '',
                'last_3mth_Oth_ratio': '',
                'last_3mth_oth_incr': '',
        ...
        dict_detail_other.update(other_dict)
        ...
        try:
            dict_result['becalled_ratio'] = float('%.2f' %(int(com_answer) / int(com_count))) if int(com_count) > 0 else ''
        except Exception as e:
            logger.error("detail add becalled_ratio is fail: %s" % (e))
            dict_result['becalled_ratio'] = ''
    ...
    #用于接口
    def vsAddressApi(self, pd_address):
        # 与通讯录匹配
        vs = DetailVsAddressApi(pd_address,
                             self.pd_detail,
                             self.phone_rank,
                             self.duration_rank,
                             self.com_valid_15_m
    ...
    #用于接口
    def vsAuthApi(self, db_auth):
        # 与通讯录匹配
        vs = DetailVsAuthApi(db_auth, self.pd_detail)
        return vs.run()
    #用于接口
    def vsInvestApi(self, db_invest_me, db_my_invest):
        # 与通讯录匹配
        vs = DetailVsInvestApi(db_invest_me, db_my_invest,  self.pd_detail)
        return vs.run()
...
class DetailVsAddressApi:
    ...
    def run(self):
        ...
        try:
            ...
            pd_duration_rank = self.duration_rank
