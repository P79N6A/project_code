...
from .model import LoanUser
from .model import LoanRepay
from .model import LoanUserLoan
from .model import LoanOverdueLoan
...
class Addrloan(object):
    ...
    def getUsers(self,lst):
        user_ids = []
        user_ids = LoanUser().getUidsByMobiles(lst)
        return user_ids
    ...
    def run(self):
        ...
        if  len(user_ids) == 0:
            return None
        user_total = LoanUser().getUidCounts(mobiles) 
        loan_all = LoanUserLoan().getAllLoanByUids(user_ids)
        loan_total = LoanUserLoan().getLoanedByUids(user_ids)
        overdue_norepay = LoanOverdueLoan().overdueAndNorepayByUids(user_ids)
        overdue_repay = LoanOverdueLoan().overdueAndRepayByUids(user_ids)
        overdue7_norepay = LoanOverdueLoan().overdue7AndNorepay(user_ids)
        overdue7_repay = LoanOverdueLoan().overdue7AndRepay(user_ids)
        last_loan_day = LoanUserLoan().lateApplyDay(user_ids)
        normal_repay = LoanRepay().advanceRepay(user_ids)
