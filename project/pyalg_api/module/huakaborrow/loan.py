...
class Loan(object):
    ...
    def __init__(self,mobile):
        db_user = LoanUser().getByMobile(mobile)
        if db_user is None:
            self.user_id = 0
        else:
    ...
    def run(self,contact):
        try:
            ...
            mobile = 0
        com_c_user = 1 if LoanUser().getUidCounts([mobile]) > 0 else 0 
        success_num = LoanRepay().getSuccessNum(self.user_id)
        last_succ_loan = LoanUserLoan().getLastSuccLoan(self.user_id)
        overdue_loan = LoanOverdueLoan().getOverdueLoan(self.user_id)
        user_loan_total = LoanUserLoan().getApplyLoan(self.user_id)
    ...
    def getIsOverdueAndIsLoading(self):
        is_overdue = LoanOverdueLoan().getIsOverdue(self.user_id)
        is_loaning = LoanUserLoan().getIsLoading(self.user_id)
        return_data = {
