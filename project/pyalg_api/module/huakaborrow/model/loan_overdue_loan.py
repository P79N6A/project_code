...
from .loan_user_loan import LoanUserLoan
import pandas as pd
...
class LoanOverdueLoan(db.Model, BaseModel):
    ...
    def overdueAndNorepayByUids(self,user_ids):
        ...
        res = db.session.query(LoanOverdueLoan).filter(LoanOverdueLoan.user_id.in_(user_ids),LoanOverdueLoan.loan_status.in_(loanStatus)).count()
        return res
    ...
    def overdue7AndNorepay(self,user_ids):
        ...
        res = db.session.query(LoanOverdueLoan).filter(LoanOverdueLoan.user_id.in_(user_ids),LoanOverdueLoan.loan_status.in_(loanStatus),LoanOverdueLoan.end_date < lastSevenday).count()
        return res
    ...
    def getOverdueLoan(self,user_id):
        ...
        datas = db.session.query(*fields).outerjoin(LoanRepay, LoanOverdueLoan.loan_id == LoanRepay.loan_id).outerjoin(LoanUserLoan, LoanOverdueLoan.loan_id == LoanUserLoan.loan_id).filter(where).all()
        if len(datas) > 0:
    ...
    def getIsOverdue(self, user_id):
        ...
        where = or_(
            LoanRepay.status == 4,
            and_(
                LoanOverdueLoan.loan_status == 7,
                LoanOverdueLoan.user_id == user_id
            )

        )
        data = db.session.query(LoanOverdueLoan.user_id).outerjoin(LoanRepay, LoanOverdueLoan.loan_id == LoanRepay.loan_id).filter(where).limit(1).first()
        if data is None:
            return 0
        return 1