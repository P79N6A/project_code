...
class LoanUserLoan(db.Model, BaseModel):
    __bind_key__ = 'spark'
    ...
    def getAllLoanByUids(self,user_ids):
        ...
        loanCounts = db.session.query(LoanUserLoan).filter(LoanUserLoan.user_id.in_(user_ids)).count()
        return loanCounts
    ...
    def getLoanedByUids(self,user_ids):
        ...
        res = db.session.query(LoanUserLoan).filter(LoanUserLoan.user_id.in_(user_ids),LoanUserLoan.status.in_(loanedStatus)).count()
        return res
    ...
    def lateApplyDay(self,user_ids):
        ...
        if len(user_ids) == 0:
            return ''
        res = db.session.query(LoanUserLoan.create_time.label('create_time')).filter(LoanUserLoan.user_id.in_(user_ids)).order_by(desc(LoanUserLoan.create_time)).first()
        if res :
    ...
    def getIsLoading(self, user_id):
        ...
        loan_status = [3, 4, 5]
        where = and_(
            LoanUserLoan.status.in_(loan_status),
            LoanUserLoan.user_id == user_id
        )
        data = db.session.query(LoanUserLoan).filter(where).limit(1).first()
        if data is None:
            return 0
        return 1
    ...
    def getApplyLoan(self, user_id):
        ...
        where = and_(
            LoanUserLoan.user_id == user_id,
            LoanUserLoan.number == 0
        )
        data = db.session.query(LoanUserLoan).filter(where).count()
        return data