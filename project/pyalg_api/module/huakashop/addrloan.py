...
from .model import ShopLoanUser
from .model import ShopLoanUserLoan
from .model import ShopLoanOverdueLoan
from .model import ShopLoanRepay
...
class Addrloan(object):
    ...
    def getUsers(self,lst):
        user_ids = []
        user_ids = ShopLoanUser().getUidsByMobiles(lst)
        return user_ids
    ...
    def run(self):
        ...
        if  len(user_ids) == 0:
            return None
        user_total = ShopLoanUser().getUidCounts(mobiles)
        loan_all = ShopLoanUserLoan().getAllLoanByUids(user_ids)
        loan_total = ShopLoanUserLoan().getLoanedByUids(user_ids)
        overdue_norepay = ShopLoanOverdueLoan().overdueAndNorepayByUids(user_ids)
        overdue_repay = ShopLoanOverdueLoan().overdueAndRepayByUids(user_ids)
        overdue7_norepay = ShopLoanOverdueLoan().overdue7AndNorepay(user_ids)
        overdue7_repay = ShopLoanOverdueLoan().overdue7AndRepay(user_ids)
        last_loan_day = ShopLoanUserLoan().lateApplyDay(user_ids)
        normal_repay = ShopLoanRepay().advanceRepay(user_ids)
