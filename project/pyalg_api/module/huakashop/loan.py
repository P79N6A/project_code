...
class Loan(object):
    ...
    def __init__(self,mobile):
        db_user = ShopLoanUser().getByMobile(mobile)
        if db_user is None:
            self.user_id = 0
        else:
    ...
    def run(self,contact):
        ...
        com_c_user = 1 if ShopLoanUser().getUidCounts([mobile]) > 0 else 0 
        success_num = ShopLoanRepay().getSuccessNum(self.user_id)
        last_succ_loan = ShopLoanUserLoan().getLastSuccLoan(self.user_id)
        overdue_loan = ShopLoanOverdueLoan().getOverdueLoan(self.user_id)
        user_loan_total = ShopLoanUserLoan().getApplyLoan(self.user_id)
    ...
    def getIsOverdueAndIsLoading(self):
        is_overdue = ShopLoanOverdueLoan().getIsOverdue(self.user_id)
        is_loaning = ShopLoanUserLoan().getIsLoading(self.user_id)
        return_data = {
