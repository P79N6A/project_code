...
class Addrloan(object):
    ...
    def getUsers(self,lst):
        user_ids = []
        user_ids = YgyUser().getUidsByMobiles(lst)
        return user_ids
    ...
    def run(self):
        ...
        if  len(user_ids) == 0:
            return None
        user_total = YgyUser().getUidCounts(mobiles)
        loan_all = YgyLoan().getAllLoanByUids(user_ids)
        loan_total = YgyLoan().getLoanedByUids(user_ids)
        overdue_norepay = YgyLoan().overdueAndNorepayByUids(user_ids)
        overdue_repay = YgyLoan().overdueAndRepayByUids(user_ids)
        overdue7_norepay = YgyLoan().overdue7AndNorepay(user_ids)
        overdue7_repay = YgyLoan().overdue7AndRepay(user_ids)
        last_loan_day = YgyLoan().lateApplyDay(user_ids)
        normal_repay = YgyLoan().advanceRepay(user_ids)
