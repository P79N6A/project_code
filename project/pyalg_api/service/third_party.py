...
class thirdParty(object):
    ...
    def runData(self,userInfo):
        ...
        #mobile = "15011284013"
        gather_info = GatherResult().getOne(mobile)
        if gather_info is None:
            return False
        userInfo['user_id'] = gather_info.user_id
    ...
    def getOtherInfo(self, userInfo):
        try:
            if userInfo is None:
                return False
            user_id = userInfo.get("user_id") #用户信息
            #mobile = userInfo.get("mobile")
            #user_id = "79770325"
            other_info = {}
    ...
    def verificationIdentity(self,user_info, data_info):
        ...
        if other_identity is None:
            other_identity = ""
        # 用户身份证信息
        user_identity = user_info.get("identity_code")
        user_realname = user_info.get("realname")
        # 判断身份证的长度
