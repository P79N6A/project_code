...
this.buildpaginate(setting)}},buildcontentimgs:function(setting){var alldivs=document.getElementById(setting.id).getElementsByTagName("div")
for(var i=0;i<alldivs.length;i++){if(this.css(alldivs[i],"contentimg","check")){setting.contentimgs.push(alldivs[i])
alldivs[i].style.display="none"}}},buildpaginate:function(setting){this.buildcontentimgs(setting)
var sliderdiv=document.getElementById(setting.id)
var pdiv=document.getElementById("paginate-"+setting.id)
var phtml=""
var toc=setting.toc
var nextprev=setting.nextprev
if(typeof toc=="string"&&toc!="markup"||typeof toc=="object"){for(var i=1;i<=setting.contentimgs.length;i++){phtml+='<a href="#'+i+'" class="toc">'+(typeof toc=="string"?toc.replace(/#increment/,i):toc[i-1])+'</a> '}
phtml=(nextprev[0]!=''?'<a href="#prev" class="prev">'+nextprev[0]+'</a> ':'')+phtml+(nextprev[1]!=''?'<a href="#next" class="next">'+nextprev[1]+'</a>':'')
pdiv.innerHTML=phtml}
var pdivlinks=pdiv.getElementsByTagName("a")
var toclinkscount=0
for(var i=0;i<pdivlinks.length;i++){if(this.css(pdivlinks[i],"toc","check")){if(toclinkscount>setting.contentimgs.length-1){pdivlinks[i].style.display="none"
continue}
...
if(setting.autorotate[0]){pdiv["onmouseover"]=function(){featuredcontentslider.cleartimer(setting,window["fcsautorun"+setting.id])}
pdiv["onmouseout"]=function(){featuredcontentslider.autorotate(setting,window["fcsautorun"+setting.id])}
sliderdiv["onmouseover"]=function(){featuredcontentslider.cleartimer(setting,window["fcsautorun"+setting.id])}
sliderdiv["onmouseout"]=function(){featuredcontentslider.autorotate(setting,window["fcsautorun"+setting.id])}
...
var totalpages=setting.contentimgs.length
var turntopage=(/prev/i.test(thepage))?currentpage-1:(/next/i.test(thepage))?currentpage+1:parseInt(thepage)
turntopage=(turntopage<1)?totalpages:(turntopage>totalpages)?1:turntopage
if(turntopage==setting.currentpage&&typeof autocall=="undefined")
return
setting.currentpage=turntopage
setting.contentimgs[turntopage-1].style.zIndex=++setting.topzindex
this.cleartimer(setting,window["fcsfade"+setting.id])
setting.cacheprevpage=setting.prevpage
if(setting.enablefade[0]==true){setting.curopacity=0
this.fadeup(setting)}
if(setting.enablefade[0]==false){setting.contentimgs[setting.prevpage-1].style.display="none"
setting.onChange(setting.prevpage,setting.currentpage)}
setting.contentimgs[turntopage-1].style.visibility="visible"
setting.contentimgs[turntopage-1].style.display="block"
...
this.setCookie("fcspersist"+setting.id,turntopage)},setopacity:function(setting,value){var targetobject=setting.contentimgs[setting.currentpage-1]
...
setting.contentimgs[setting.cacheprevpage-1].style.display="none"
...
if(setting.cacheprevpage!=setting.currentpage){setting.contentimgs[setting.cacheprevpage-1].style.display="none"}}},css:function(el,targetclass,action){var needle=new RegExp("(^|\\s+)"+targetclass+"($|\\s+)","ig")
if(action=="check")
