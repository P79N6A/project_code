...
//         }
	...
	//POST方式直接提交数据
    public static function dataPost($content, $url, $timeout=15) {
        /*$context = array('http' => array('method' => "POST", 'header' => "Content-type: application/x-www-form-urlencoded", 'content' => $content));
    ...
    public static function sendTemplatePost($post_content, $url) {
        ...
        $data = file_get_contents($url, FALSE, $stream_context);
        return $data;*/
    	$post_content = http_build_query($post_content);
    	return self::dataPost($post_content, $url);
    }
