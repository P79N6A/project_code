 ...
 */
class SfLoan extends BaseDBModel
{
    /**
     * @inheritdoc
     */
    ...
    {
        return [
            [['user_id', 'contract', 'contract_url', 'bank_id'], 'required'],
            [['user_id', 'days', 'type', 'status', 'business_type', 'bank_id', 'come_from', 'from_code', 'is_calculation', 'version'], 'integer'],
            [['amount', 'need_repay_amount', 'repay_amount', 'interest_fee', 'withdraw_fee'], 'number'],
            [['start_date', 'end_date', 'withdraw_time', 'repay_time', 'modify_time', 'create_time'], 'safe'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'end_date' => '到期日',
            'type' => '借款类型：1普通借款',
            'status' => '借款状态:1初始；2审核通过；3审核驳回；4失效；5申请提现；6申请提现通过；7申请提现驳回；8已还款；9出款成功；10出款失败；11待确认还款；12还款异常(未还款逾期)；13 还款异常(部分还款 逾期)；14 超额还款(借款完成)；15 异常提现驳回；20出款中；',
            'interest_fee' => '借款利息总额',
            'desc' => '借款说明',
            'contract' => '合同编号',
            'contract_url' => '合同存放地址',
