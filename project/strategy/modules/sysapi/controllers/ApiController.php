...
class ApiController extends Controller
{
    ...
    protected function error($rsp_code,$rsp_msg,$postdata)
    {
        $returnData = [
            'rsp_code' => $rsp_code,
            'rsp_msg' => $rsp_msg,
    ...
    protected function success($res_info,$postdata)
    {
        $returnData = [
            'rsp_code' => isset($res_info['rsp_code']) ? $res_info['rsp_code'] : '0000',
            'rsp_msg' => isset($res_info['rsp_msg']) ? $res_info['rsp_msg'] : 'success',
            'req_id' => isset($postdata['req_id']) ? $postdata['req_id'] : '',
            'user_id' => isset($postdata['user_id']) ? $postdata['user_id'] : '',
            'loan_id' => isset($postdata['loan_id']) ? $postdata['loan_id'] : '',
            'aid'=> isset($postdata['aid']) ? $postdata['aid'] : '',
            'result' => isset($res_info['RESULT']) ? $res_info['RESULT'] : '3',
            'Insure_fee' => isset($res_info['Insure_fee']) ? $res_info['Insure_fee'] : '0.18',
            'rollover_money' => isset($res_info['Rollover_money']) ? $res_info['Rollover_money'] : '0',
            'rollover_fee' => isset($res_info['Rollover_fee']) ? $res_info['Rollover_fee'] : '0.05',
        ];
        $ret_info = (new ApiSign)->signData($returnData);
        echo json_encode($ret_info, JSON_UNESCAPED_UNICODE);
        exit;
    ...
    protected function suc($res_info,$postdata)
    {
        $def_money = round(0.18 * $postdata['amount']);
        $returnData = [
            'rsp_code' => isset($res_info['rsp_code']) ? $res_info['rsp_code'] : '0000',
            'rsp_msg' => isset($res_info['rsp_msg']) ? $res_info['rsp_msg'] : 'success',
            'req_id' => isset($postdata['req_id']) ? $postdata['req_id'] : '',
            ...
            'loan_id' => isset($postdata['loan_id']) ? $postdata['loan_id'] : '',
            'aid'=> isset($postdata['aid']) ? $postdata['aid'] : '',
            'result' => isset($res_info['SCORE_STATUS']) ? $res_info['SCORE_STATUS'] : '3',
            'ext_result' => isset($res_info['EXTENSION_RESULT']) ? $res_info['EXTENSION_RESULT'] : '3',
            'Insure_fee' => isset($res_info['Insure_fee']) ? $res_info['Insure_fee'] : '0.18',
            'rollover_money' => isset($res_info['Rollover_money']) ? $res_info['Rollover_money'] : $def_money,
            'rollover_fee' => isset($res_info['Rollover_fee']) ? $res_info['Rollover_fee'] : 0,
        ];
        $ret_info = (new ApiSign)->signData($returnData);
        echo json_encode($ret_info, JSON_UNESCAPED_UNICODE);
        exit;
