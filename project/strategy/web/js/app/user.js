...
$(function(){
	...
	$('#reggetcode').click(function () {
		...
		$.post("/app/reg/onesend",{mobile:mobile},function(result){
			...
			}else if(data.ret == '2')
			{
				...
				$.Zebra_Dialog('该手机号已注册，请<a href="/app/reg/login">登录</a>', {
				    'type':     'question',
				    'title':    '手机验证',
				    'buttons':  [
//				                    {caption: '取消', callback: function() {}},
	...
	$('#reggetcode_login').click(function () {
		...
		$.post("/app/reg/loginsend",{mobile:mobile},function(result){
			var data = eval("("+ result + ")" ) ;
			if( data.ret == '0' ){
				//发送成功
				count = 60 ;
			...
			}else if(data.ret == '2')
			{
				...
				$.Zebra_Dialog('手机号不存在，去<a href="/app/reg/one">注册</a>', {
				    'type':     'question',
				    'title':    '手机验证',
				    'buttons':  [
//				                    {caption: '取消', callback: function() {}},
	...
	$("#regonebutton").click(function(){
		...
		var agree_xieyi = $("#agree_reg_xieyi").is(":checked");
		if(agree_xieyi)
		{
			$.post("/app/reg/onesave",{mobile:mobile,code:code,from_code:from_code,user_type:user_type,url_type:url_type},function(result){
				var data = eval("("+ result + ")" ) ;
				if( data.ret == '0' ){
					if( data.url != '' ){
	...
	$("#login_button").click(function(){
		...
		$.post("/app/reg/loginsave",{mobile:mobile,code:code},function(result){
			var data = eval("("+ result + ")" ) ;
			if( data.ret == '0' ){
				if( data.url != '' ){
					window.location = data.url ;
				...
				$("#reg_one_error").html('手机号不存在，去<a href="/app/reg/one">注册</a>');
			}else{
				$("#reg_one_error").html('验证码错误');
			}
	...
	$("#loan_confirm").click(function(){
		...
		if(agree_xieyi)
		{
			...
			$.post("/app/loan/confirm", {desc:desc,days:days,amount:amount,coupon_id:coupon_id,coupon_amount:coupon_amount,bank_id:bank_id}, function(result){
				var data = eval("("+ result + ")" ) ;
				//alert(data);
				if(data.ret == '3')
				{
	...
	$("#loan_conwd").click(function(){
		var agree_xieyi = $("#agree_loan_xieyi").is(":checked");
		if(agree_xieyi)
		{
			var loan_id = $("input[name='loan_id']").val();
			$("#loan_conwd").attr('disabled', true);
			$.post("/app/loan/conwd", {loan_id:loan_id}, function(result){
				var data = eval("("+ result + ")" ) ;
				window.location = data.url ;
			});
		}
	...
	$("#reg_fromcode_button").click(function(){
		...
		$.post("/app/invite/invitesave",{code:code},function(result){
			var data = eval("("+ result + ")" ) ;
			if( data.ret == '0' ){
				window.location = '/app/reg/one?code='+code ;
			}else if( data.ret == '3'){
	...
	$("#find_friend").click(function(){
		...
		window.location = "/app/share/share?mobile="+mobile;
	});
	
	$("#invest_friend").click(function(){
		window.location = "/app/invest/detail";
	});
	
	$("#invest_detail").click(function(){
		window.location = "/app/invest/confirm";
	});
	...
	$("#invest_confirm").click(function(){
		...
		if(agree_xieyi)
		{
			...
			$.post("/app/invest/addsave", {loan_id:loan_id,input_amount:input_amount}, function(data){
				if(data == 'fail')
				{
					alert('投资失败')
				...
				{
					window.location = "/app/invest/success?invest_id="+data;
				}
			});
		}
		else
	...
	$("#reg_two_form").click(function(){
		...
		$.post("/app/reg/twosave",{school:school,school_name:school_name,edu:edu,school_time:school_time,realname:realname,identity:identity,f_url:f_url},function(result){
			var data = eval("("+ result + ")" ) ;
			if( data.ret == '0' ){
				if( data.url != '' ){
	...
	$("#reg_shtwo_form").click(function(){
		...
		$.post("/app/reg/shtwosave",{industry:industry,company:company,position:position,realname:realname,identity:identity,f_url:f_url},function(result){
//			alert(result);
//			debugger;
			var data = eval("("+ result + ")" ) ;
			if( data.ret == '0' ){
	...
	$("#get_user_headurl").click(function(){
		$.post("/app/account/getuserinfo", {}, function(data){
			window.location = "/app/account/info";
		});
	});
	$("#reg_shthree_form").click(function(){
		...
		$.post("/app/reg/shthreesave",{school:school,school_name:school_name,edu:edu,school_time:school_time,f_url:f_url},function(result){
//			alert(result);
//			debugger;
			var data = eval("("+ result + ")" ) ;
			if( data.ret == '0' ){
	...
	$("#reg_pic_button").click(function(){
		var chooseImage = $("input[name='chooseImage']").val();
