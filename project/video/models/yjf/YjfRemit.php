 ...
 */
class YjfRemit extends \app\models\BaseModel {
    ...
    public function rules() {
        return [
            [
                ...
                ['req_id','third_orderid'], 'string', 'max' => 40], [
    ...
    public function saveRemitData($postData) {
        ...
        $data = [
            ...
            'third_orderid' => isset($postData['third_orderid']) ? $postData['third_orderid'] : '',
            'client_id' => $client_id,
   ...
   /**
     ...
     * @param $oid_paybill 第三方支付订单号
     * @param $sub_remit_time 出款时间
     * @param $source 1: 出款调用; 2:查询调用;3:异步通知调用
     * @return bool
     */
    public function saveRspStatus($remit_status, $rsp_status, $rsp_status_text,$oid_paybill, $sub_remit_time, $source) {
       // 当是出款中时, 更新下次的查询时间
        if ($source == 2) {
            // 累加查询次数
            $this->query_num++;
        ...
        if(!empty($oid_paybill)){
            $this->third_orderid = $oid_paybill;
        }
...
//        print_r($data);
    ...
    public function InputNotify(YjfRemit $oRemit)
    {
    ...
    private function addNotify(YjfRemit $oRemit)
    {
        if (in_array($oRemit['remit_status'], [YjfRemit::STATUS_SUCCESS, YjfRemit::STATUS_FAILURE])) {
            ...
            if (!$result) {
                Logger::dayLog('yjfback', 'YjfRemit/addNotify', 'YjfClientNotify/saveData', $oClientNotify->errors);
                return false;
            }
        }
        return true;
     ...
     * @return boolean
     */
    public static function isFinished(YjfRemit $oRemit){
        if (in_array($oRemit['remit_status'], [YjfRemit::STATUS_SUCCESS, YjfRemit::STATUS_FAILURE])) {
            return true;
        }
        return false;
