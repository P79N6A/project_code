...
class RbCreditApi {
     ...
     * @return string
     */
    private function RSADecryptkey($encryptKey){
     ...
     * @return void
     */
    public function send($paramArr,$flag = 1){
        //创建签名
        ...
        //请求url
        if($flag == 1) 
            $url = $this->config['commitUrl'];
        else
            $url = $this->config['queryUrl'];
        Logger::dayLog('rbcredit','rbcredit/send', '请求数据',$paramArr,$generateAESKey, $url, $requestData);
        ...
        if($resp){
            ...
            Logger::dayLog('rbcredit','rbcredit/send', '返回数据',$paramArr, $url, $resData);
        }
        return $resData;
    }
