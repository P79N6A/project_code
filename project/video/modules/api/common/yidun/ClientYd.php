...
class ClientYd{
    ...
    public function updateRequestRow($requestid,$resInfo){
        ...
        $jxlModel = $jxlModel->getById($requestid);
        if(!$jxlModel){
            return ['res_code'=>25092, 'res_data'=>'数据异常'];
        }
        $jxlModel->process_code = $resInfo->code;
        $resjxl = $jxlModel->save();
        if (!$resjxl) {
            Logger::dayLog('yidun','updateJxlCode:更新数据失败');
        }
...
//定时批次请求上数查询接口获取详单、报告
    ...
    public function queryYidun($pullData){
        ...
        $oHistory = $oJxlStat->getHistoryNew($request->phone);
        if ($oHistory) {
