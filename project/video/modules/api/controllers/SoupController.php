...
class SoupController extends ApiController
{
    ...
    public function init() {
        ...
        $params['_sign'] = $oApiCrypt->sign($params, '90CdDbCiMc328kMkea3849r34a7YGY');
        $a = Crypt3Des::encrypt(json_encode($params), '90CdDbCiMc328kMkea3849r34a7YGY');
        var_dump($a);exit;
        */
        parent::init();
    }
    ...
    public function actionIdcard()
    {
        ...
        $image_data = [
            ...
            'aid'               => ArrayHelper::getValue($post_data, 'aid'),
        ];
    ...
    /**
     * 静默活体检测  soup/stateless
     * 需要参数：
     *      video_file  视频地址
     */
    public function actionStateless()
    {
        //1.
        $post_data = $this->reqData;
...
//        $params = [
//            'video_file'    => 'http://img.xianhuahua.com//yiyiyuan/transfer/2017/11/30/1508024494.jpg',
//        ];
        $result = $oStateless->moveFile($post_data);
        $pic_code = ArrayHelper::getValue($result, 'code');
        //var_dump(ArrayHelper::getValue($result, 'msg'));
        return $this->resp($pic_code, ArrayHelper::getValue($result, 'msg'));
    }
