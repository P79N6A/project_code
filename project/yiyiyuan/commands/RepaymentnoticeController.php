 ...
 */
class RepaymentnoticeController extends BaseController {
    ...
    public function actionIndex() {
        ...
        $condition = [
            'AND',
            [">", User_loan::tableName() . ".end_date", $start_time],
            ["<", User_loan::tableName() . ".end_date", $end_time],
        ...
        for ($i = 0; $i < $pages; $i++) {
            $user_loan = User_loan::find()
                    ...
                    ->select(array(User_loan::tableName() . '.user_id', User_loan::tableName() . '.loan_id', User_loan::tableName() . '.status', User_loan::tableName() . '.end_date', User_loan::tableName() . '.create_time', User_loan::tableName() . '.amount', User_loan::tableName() . '.current_amount', User_loan::tableName() . '.interest_fee', User_loan::tableName() . '.withdraw_fee', User_loan::tableName() . '.collection_amount', User_loan::tableName() . '.coupon_amount', User_loan::tableName() . '.like_amount', User_loan::tableName() . '.chase_amount', User::tableName() . '.mobile', User::tableName() . '.realname'))
                    ->where($condition)
                    ->offset($i * $limit)
                    ->limit($limit)
                    ->all();
            ...
            foreach ($user_loan as $key => $value) {
                ...
                $value->chase_amount = $newLoanModel->getChaseamount($value->loan_id);
                //应还款金额
                $huankuan_amount = $value->getRepaymentAmount($value, 2);
                //借款时间
                //$loan_time = date('m' . '月' . 'd' . '日', strtotime($value->create_time));
                if ($value->end_date) {
                    $end_time = date('Y-m-d', strtotime('-1 days', strtotime($value->end_date)));
                } else {
                    $end_time = '';
                }
...
//        $openApi = new Apihttp;
        //$result = $openApi->sendSmsByChuanglan($postdata);
        $apiModel = new ApiSms();
