...
class GetsessionController extends BaseController {
    public function actionIndex() {
        ...
        if (empty($o_access_token) || empty($is_invalid)) {
            Logger::dayLog('script/sobot/getsession', 'token不存在or失效', $o_access_token, $is_invalid);
            exit('token error');
        }
        ...
        if (empty($token) || $token['code'] != '1000' || empty($token['data']) || empty($token['data']['item'])) {
            Logger::dayLog('script/sobot/getsession', '接口错误', $token_json, $token);
            exit('error');
        }
        $data = [
            'startDate' => $startDate,
        ...
        $o_session = (new SobotSession())->getByStartAndEnd($startDate, $endDate);
        if (!empty($o_session)) {
            $result = $o_session->updateRecord($data);
            if (empty($result)) {
                Logger::dayLog('script/sobot/getsession', '更新失败', $data, $result);
                exit('yi_access_token error');
            }
        } else {
            $result = (new SobotSession())->addRecord($data);
            if (empty($result)) {
                Logger::dayLog('script/sobot/getsession', '记录失败', $data, $result);
                exit('yi_access_token error');
            }
        }
        exit('success');
