 ...
 * sysloanguide/renewalrepay/index
 */
// 这个在计划任务里面避免超时或者内容不够
set_time_limit(0);
ini_set('memory_limit', '-1');

class RenewalrepayController extends BaseController {
	...
	public function actionIndex($startTime = '', $endTime = '') {
		...
		$where = [
            'and',
            ['>=', 'qj_renewal_payment_record.last_modify_time',$startTime],
            ['<',  'qj_renewal_payment_record.last_modify_time', $endTime],
            ['=',  'qj_renewal_payment_record.status',      1],
			['>', 'b.id', 0],
        ];
		$renewal_payment	 = (new Renewal_payment_record_guide())->find()->leftJoin('qj_overdue_loan AS b', 'qj_renewal_payment_record.loan_id = b.loan_id')->where($where)->select('qj_renewal_payment_record.*')->all();
		...
		foreach ($renewal_payment_list as $rk => $rv) {
			...
			if ($resultArr['rsp_code'] != '0000') {
				Logger::dayLog('renewalrepay', '同步还款记录失败', $data);
			}
			Logger::dayLog('renewalrepay/returnloanids', '同步还款数据loanID记录', '开始时间为' . $startTime, '结束时间为' . $endTime, '条数' . count($ids), $ids);
		}
	}
