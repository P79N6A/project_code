...
class UpdatechaseamountController extends BaseController {
	...
	public function actionIndex() {
		...
		$count		 = (new OverdueLoan())->find()->where($where)->count();
		$id			 = 0;
		$limit		 = 500;
		...
		for ($i = 1; $i <= $forcount; $i++) {
			...
			$postData = $data = [];
			$info = (new OverdueLoan())->find()->where($where)->orderBy('id asc')->limit($limit)->all();
			...
			if ($resultArr['rsp_code'] != '0000') {
				Logger::dayLog('updatechaseamount', '同步逾后还款记录失败', $postData);
			}
		}
	}
