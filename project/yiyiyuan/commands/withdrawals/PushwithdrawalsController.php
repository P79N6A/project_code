...
/**
 *  5天未体现债匹推送
 *  linux : sudo -u www /data/wwwroot/yiyiyuan/yii withdrawals/pushwithdrawals
 *  windows D:phpStudy\php56n\php.exe D:WWW\yiyiyuana_app\yii withdrawals/pushwithdrawals
 */
...
class PushwithdrawalsController extends BaseController {

    private $limit = 200;

    public function actionIndex() {
        $time_start = date("Y-m-d H:i:00");
        $time_end = date('Y-m-d H:i:00', strtotime('-5 day'));
        $where = [
            'AND',
            ['>=', Cg_remit::tableName() . '.last_modify_time', $time_end],
            ['<',Cg_remit::tableName() . '.last_modify_time',$time_start],
            [Cg_remit::tableName() . '.remit_status' => 'NOREMIT'],
