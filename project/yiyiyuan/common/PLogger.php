...
Class PLogger {
    ...
    /**
     * PLogger constructor.
     * @param $source ios、android、web
     * @param $user_id
     * @param string $type
     * @param string $service_type
     */
    private function __construct($source, $type = '', $user_id, $service_type = '') {
        static::$log_data = $this->logstart($source, $user_id, $service_type);
     ...
     * @return PLogger|null
     */
    public static function getInstance($source, $type = '', $user_id, $service_type = '') {
        if (static::$instance) {
            return static::$instance;
        }
        return static::$instance = new self($source, $type, $user_id, $service_type);
    }
    ...
    public static function getJson() {
        ...
        if ($data) {
            ...
            $json_data['log_url'] = Yii::$app->params['log_url'];
        }
//        print_r($json_data);die;
    ...
    public function logend($type = '') {
        ...
        $data = static::$log_data;
        if (empty($data)) {
            Logger::dayLog("files/yiyiyuan_fulllog/txt", '写入data为空');
        }
        ...
        $data['g_uid'] = $data['g_uid'] ? Crypt3Des::decrypt($data['g_uid']) : '';
        return static::dayLogCsv('files/yiyiyuan_fulllog/' . $data['source'], $data);
    }
     ...
     * @return array
     */
    public function logstart($source, $user_id, $service_type) {
        //获取用户信息
        $user = '';
        if ($source == 'weixin') {
            $user = Yii::$app->user->identity;
            if (empty($user)) {
                $user = (new User())->getUserInfoByUserId($user_id);
            }
        } elseif ($source == 'webs') {
            $user = Yii::$app->user->identity;
            if (empty($user)) {
        ...
        } elseif ($source == 'android' || $source == 'ios') {
            if (!empty($user_id)) {
                $user = (new User())->getUserinfoByUserId($user_id);
            }
        }
        ...
        $header['g_uid'] = !empty($user) ? Crypt3Des::encrypt($user->user_id) : '';
        $header['is_login'] = !empty($user) ? 'yes' : 'no';
        $header['channelid'] = '';
        $header['activity'] = '';
        ...
        if (!empty($service_type)) {
            $header['url'] = $this->getUrl() . '&service_type=' . $service_type; //当前url
        } else {
        ...
        $header['uuid'] = '';
        if ($source == 'weixin') {
            if (empty($user) || empty($user->openid) || empty($user->userwx)) {
                $header['nickname'] = '';
                $header['sex'] = '';
                $header['area'] = '';
                //只授权未登录
                ...
                $userwx = $user->userwx;
                $header['nickname'] = !empty($userwx) ? preg_replace('/\s/','',addslashes(htmlspecialchars($userwx->nickname,ENT_QUOTES))) : '';
                $header['sex'] = '';
                $header['area'] = '';
                //只授权未登录
                ...
                if ($access_token && !empty($header['openId'])) {
                    ...
                    if (!empty($user_info) && !array_key_exists('errcode', $user_info)) {
                        if (isset($user_info['sex']) && $user_info['sex'] == 1) {
                            $sex = '女';
                        } elseif (isset($user_info['sex']) && $user_info['sex'] == 2) {
                            $sex = '男';
                        } else {
                            $sex = '未知';
                        }
                        $header['nickname'] = isset($user_info['nickname']) ? preg_replace('/\s/','',addslashes(htmlspecialchars($user_info['nickname'],ENT_QUOTES))) : '';
                        $header['sex'] = $sex;
                        $header['area'] = isset($user_info['city']) ? $user_info['city'] : '';
                        //授权并登录
