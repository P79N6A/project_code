...
//	正式环境请求基础地址
     ...
     */
    public function bindPayRequest($bind_id, $order_id, $transtime, $amount, $product_catalog, $identity_id, $identity_type, $user_ip, $terminaltype, $terminalid, $callbackurl = '', $fcallbackurl = '', $currency = 156, $product_name = '', $product_desc = '', $other = '') {
        $query = array(
    ...
    protected function parseReturnData($data) {
        $return = json_decode($data, true);
        if (empty($return)) {
            throw new yeepayMPayException('no response', 1);
        }
