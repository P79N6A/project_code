...
class RSA {
     ...
     * @return 签名值
     */
    public function sign($data, $private_key, $code = 'base64', $openssl_algo = OPENSSL_ALGO_SHA1) {
        $private_key = $this->setPrikey($private_key);
        $ret = false;
        if (openssl_sign($data, $ret, $private_key, $openssl_algo)) {
     ...
     * @return bool
     */
    public function verify($data, $sign, $public_key, $code = 'base64', $openssl_algo = OPENSSL_ALGO_SHA1) {
        $public_key = $this->setPubKey($public_key);
        $ret = false;
        $sign = $this->_decode($sign, $code);
        if ($sign !== false) {
            switch (openssl_verify($data, $sign, $public_key, $openssl_algo)) {
                case 1 :
                    $ret = true;
                    break;
                case 0 :
    ...
    /**
     * java加密会替换-_,所以要替换回来之后进行解密,php联调好像不需要使用这个方法
     * @param type $data
     * @return type
     */
