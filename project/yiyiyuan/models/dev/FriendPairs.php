...
*/
class FriendPairs extends \app\models\dev\Friends
{
	/**
	 * 会员id
	 */
	...
	/**
	 * 获取两两点赞关系
	 * @todo 这个做错了 应该是user_share_click
	 */
	public function getLikeRelation(){
		$loanTable = User_loan::tableName();
		$recordTable = Loan_record::tableName();
		
		return User_loan::find()-> leftJoin("{$recordTable}","{$loanTable}.loan_id={$recordTable}.loan_id") 
						  		-> where([
						  			"{$loanTable}.user_id"=>$this->fuser_id, // 借款表里面的是被投资的
						  			"{$recordTable}.invest_user_id"=>$this->user_id, // record表里面是投资人
						  			])
						  		-> exists();
	}
	/******************************end 两个好友关系******************************/
