...
class XsApiReturn{
     ...
     * 获取借款计算结果
     */
    public function getLoan($oBasic){
        //1 基本数据
        if(!$oBasic){
            return null;
        }
     ...
     * @return []
     */
    private function getCommon($oBasic){
        //1. 获取基本数据
        ...
        //6. 获取同盾数据;同盾单独键值存放
        $fraudMetrix = $this->getFraudMetrix($oBasic['phone'],$oBasic['idcard'],$oBasic["event"]);
        $data['fm'] = $fraudMetrix;
        //7. 获取百度数据；百度数据单独存放
     ...
     * 获取同盾信息
     */
    private function getFraudMetrix($phone,$idcard,$event){
        ...
        $fm_data = [];
        $oFm = (new XsFraudmetrix) -> getResult($phone,$idcard,$event);
        if($oFm){
