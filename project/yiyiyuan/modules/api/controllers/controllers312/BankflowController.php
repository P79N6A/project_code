...
class BankflowController extends ApiController {
    ...
    public function actionIndex() {
        ...
        if ( ($res['res_code'] != '105002' && $res['res_code'] != '0') || empty($res['res_url'])) {
            Logger::dayLog('app/bankflow', '认证接口请求失败' . $user_id, $res);
        ...
        if (empty($save_result)) {
            Logger::dayLog('app/bankflow', '请求认证之后存储数据yi_selection失败' . $user_id, $save_result);
