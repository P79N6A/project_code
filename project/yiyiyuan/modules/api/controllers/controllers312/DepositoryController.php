    ...
    public function actionIndex() {
        ...
        if ($is_repay == 1) {
            if (empty($isfourinoneAuth) || !$o_pay_account_extend->getLegal(1)) {
                if ((empty($isfundAuth) || empty($isrepayAuth)) && empty($isfourinoneAuth)) {
                    $returnCustodyData['type'] = 5;
                ...
                if(!empty($isrepayAuth)){
                    $repayTime = $isrepayAuth->isTimeOut();
                }
                if(!empty($isfundAuth)){
                    $fundTime = $isfundAuth->isTimeOut();
                }
                if(!empty($o_pay_account_extend)){
                    $fourinoneTime = $o_pay_account_extend->getLegal(1);
                }
                if ($repayTime || $fundTime || !$fourinoneTime) {
