...
class RepayController extends ApiController {
    ...
    public function actionIndex() {
        ...
        if (in_array($loaninfo['business_type'], [5,6,11])) {
            ...
            $overdue=!empty($overdueLoan) ? TRUE : FALSE;
        } else {
            $overdue = (in_array($loaninfo->status, [12, 13])) ? TRUE : FALSE;
        }
        //判断是否是分期并且是分期还款
        if (in_array($loaninfo['business_type'], [5,6,11]) && $type==1) { //分期
            ...
            $pay_account = (new Loan_repay())->isDepositoryRepaytemrs($loaninfo, $user, $bank, $repayAmountA, $overdue, $goodbill_arr, $lastperiod);
            if ($pay_account) {
                $platform = 26;//存管通道
            ...
            //还款记录
            $oLoanRepay = $this->repayLoan($loaninfo, $repay_amount, $bank_id, $source, $platform);
            if (!$oLoanRepay) {
            ...
            // 请求支付前 锁定到待支付
            $locktorepay =(new StageService())->locktorepay($oLoanRepay['id'],$is_repay_arr);
            $callbackurl = Yii::$app->params['newdev_notify_url'];
            $order_id = $oLoanRepay->repay_id;
        } elseif ($type == 1) {
        ...
        $post_data = $this->pay($bank, $user, $order_id, $repayAmountA, $callbackurl, $loaninfo, $overdue, $account_id, $couponVal, $type, $lastperiod);
        if ($post_data) {
     ...
     * 请求支付
     */
    private function pay($bank, $user, $orderid, $money, $callbackurl, $loaninfo = '', $overdue = false, $account_id = '', $couponVal, $type, $lastperiod) {
