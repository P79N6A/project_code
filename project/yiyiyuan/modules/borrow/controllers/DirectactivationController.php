...
class DirectactivationController extends BorrowController
{
    ...
    public function actionActivating(){
       ...
       // Logger::dayLog('weixin/directactivation', 'redict_activation_num',$loaninfo->user->user_id, $redict_activation_num); //@todo 监测使用，后期请删除
    ...
    public function actionActivationresult(){
        ...
        if($info['channel'] == 1){
         ...
         if($activation_num < 3){
             $btn_status = 1;
             $try_activation_url = '/borrow/loan?loan_id='.$loan_id;
