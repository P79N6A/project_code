...
<div class="haikfshi"><a href="/new/repay/repaychoose?loan_id=<?php echo $loan['loan_id']; ?>" id="demo16">全额还款</a></div>
...
<div class="xuqhk" <?php if($is_show != 1): ?>style="display: none;" <?php endif; ?>>
	<h3>什么是续期还款？</h3>
	<p>续期还款指用户在最后还款日前通过支付一定费用将最后还款日延长避免借款逾期影响个人信用。</p>
	<h3>续期还款是否收费？</h3>
...
<div class="ttfukfsi checkcard" style="display: none;" >
    ...
    <div class="haimoneys">
        <p class="haitxts"><?php echo $money; ?> <em>元</em></p>
    </div>
    <div class="tuika">
    ...
    <div class="tuika" style="height: 14rem;overflow:auto">
        ...
        <a <?php if($val['sign'] == 2): ?>class="check_bank"<?php endif; ?> bank_id='<?php echo $val['id'] . "|" . getImageUrl($val['bank_abbr']) . "|" . $val['bank_name'] . "|" . substr($val['card'], strlen($val['card']) - 4, 4) . "|" . $val['type']; ?>' style="<?php if ($val['sign'] == 1): ?>background:#e7e7e7;<?php endif; ?>position: relative;">
                <div class="bank_nn"> 
                    ...
                    <div class="sendtwo"><p><?php if($val['bank_abbr'] == 'GDB'): echo "广发银行";  else: echo $val['bank_name']; endif; ?> <span><?php echo $val['type'] == 0 ? '借记卡' : '信用卡'; ?></span> <em>尾号<?php echo substr($val['card'], strlen($val['card']) - 4, 4); ?></em></p> </div>
                    <?php if ($val['sign'] == 1): ?>
                        <img class="zbzchi" src="/images/zanbuzhichi2.png">
                    <?php else: ?>
                        <img class="rightjt" src="/images/rightjt.png">
...
<script>
    ...
    $(function() {
        ...
        $('#is_submit').click(function() {
            ...
            $.post("/dev/renewal/subpay", {bank_id: bank_id, loan_id: loan_id}, function(result) {
...
//        $(".chakan").click(function() {
        ...
        $('#submit').click(function() {
            ...
            if (channel === 'weixin') {
                ...
                $.post("/dev/renewal/weixinsubpay", {loan_id: loan_id}, function(result) {
                    if (result.status == '0') {
