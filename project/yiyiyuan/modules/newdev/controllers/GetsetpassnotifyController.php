...
class GetsetpassnotifyController extends NewdevController{
    ...
    public function actionIndex() {
        $postData = $this->post();
        Logger::errorLog(print_r($postData, true), 'PasswordNotify', 'debt');
        if(!is_array($postData) || !isset($postData['data'])){
            exit('error1');
        }
        $result = json_decode($postData['data'], true);
        Logger::errorLog(print_r($result, true), 'PasswordNotify', 'debt');
