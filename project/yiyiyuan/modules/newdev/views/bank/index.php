            ...
            <ul>
                <?php foreach ($banks as $key => $val): ?>
                    <li class="border_top_2">
                        <a href="/new/bank/delbank?id=<?php echo $val['id']; ?>">
                            ...
                            <div class="col-xs-7 bank_cont">
                                <span class="n36 bankN"><?php echo empty($val['bank_name']) ? '银行卡' : $val['bank_name']; ?></span>
                                <i class="redLight"><?php if(!empty($val['bank_name'])){echo $val['type'] == 0 ? '借记卡' : '信用卡';} ?></i>
                                <div class="clearfix"></div>
                                <p class="n24 grey4 mt10">
                                    尾号<?php echo substr($val['card'], strlen($val['card']) - 4, 4) ?>
                                    <?php if($val['default_bank'] && $val['default_bank'] == 1){?>
                                        <span style="padding-left: 60px">默认卡</span>
                                    <?php } ?>
                                </p>
                            </div>
    ...
    <div class="main">
        <?php if (count($banks) < 10): ?>
            <div class="addCard mt20 <?php echo count($banks) > 0 ? 'nBackg' : '' ?>">
                <a href="/new/bank/addcard?banktype=3&orderinfo=<?php echo urlencode($orderInfo); ?>"><span>添加银行卡</span></a>
            </div>
        <?php else : ?>
        ...
        <?php endif; ?>
        <a href="/new/bank/quota" class="n22 float-right aColor mt20">支持银行卡及限额说明</a>
    </div>
    <div class="bottomBtn n36 red text-center"><a href="/new/account"><span>返回账户页</span></a></div>
</div>
