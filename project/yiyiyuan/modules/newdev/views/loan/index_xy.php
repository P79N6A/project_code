...
<!--     <ul class="nav_jk overflow">
        <li><div class="item on" onclick="loanChange(1)"><a>信用借款</a></div></li>
        <li><div class="item" onclick="loanChange(4)"><a>担保借款</a></div></li>
    </ul> -->
</div>
...
<!--                </div>-->
    ...
    <form class="form-horizontal" role="form" method="post" action="/new/loan/second">
        <input id="business_type" type="hidden" name="business_type" value="1"/>
    ...
    <?php if ($reject_data['is_reject'] == 1 && !empty($reject_data['reject_data'])): ?>
        ...
        <div class="duihsucc bohomeg">
            ...
            <?php else: ?>
                <?php if ($reject_data['is_selection'] == 1): ?>
                    <div>
                        <button class="yesknow" style="font-size:14px;float: left; margin-left: 7%;" onclick="supplement()">立即完善资料</button>
                        <button class="yesknow iknow" style="font-size:14px;float: left; margin-left: 5%;">我知道了</button>
                    </div>
                <?php else: ?>
...
<script>
    ...
    function supplement() {
        tongji('supplement',baseInfoss);
        setTimeout(function(){
            window.location.href = '/new/account/peral';
        },100);
    }
</script>
