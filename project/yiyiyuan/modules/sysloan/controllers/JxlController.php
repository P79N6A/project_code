...
use app\commonapi\JxldownSysloan;
use app\models\news\User;
use app\models\news\User_password;
...
class JxlController extends ApiController {
	...
	public function actionReporttotal() {
		$required	 = ['phone'];  //必传参数
		...
		//1. 获取电商数据
		$obj = new JxldownSysloan($phone,'',$number);
		$res = $obj->getRes();
		$total = empty($res['total']) ? 0 : intval($res['total']);
		$array['rsp_code']	 = '0000';
		$array['rsp_msg']	 = '成功';
		$array['data']		 = $total;
		exit(json_encode($array));
	...
	public function actionReportall() {
		$required	 = ['phone','number'];  //必传参数
		$httpParams	 = $this->post();  //获取参数
		$verify		 = $this->BeforeVerify($required, $httpParams);
		$phone		 = $httpParams['phone'];
		$number		 = $httpParams['number']> 1 ? $httpParams['number'] : 1;
		if (!isset($phone) || empty($phone)) {
			$array['rsp_code']	 = '1000';
			$array['rsp_msg']	 = '缺少手机号码';
			exit(json_encode($array));
