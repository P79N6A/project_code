...
class UserController extends ApiController {
	...
	public function actionGetcontacts() {
		...
		$favorite = Favorite_contacts::find()
				->where(['user_id' => $httpParams['user_id']])
				->select('mobile,contacts_name,relatives_name,phone')
				->asArray()->one();
		if (empty($favorite)) {
	...
	public function actionSetblack() {
		...
		$result_log = Common::saveLog('UserController', 'actionSetblack', $ip, 'sysloan', $httpParams['user_id']);
		/*		 * *************记录访问日志end**************** */


		$fuser = (new User)->getUserinfoByUserId($httpParams['user_id']);
		if (empty($fuser)) {
			$array = $this->errorreback('10001');
			exit(json_encode($array));
		}
	...
	public function actionGetaddresslist() {
		...
		/*		 * *************记录访问日志beigin************* */
		$ip = Common::get_client_ip();
		$result_log = Common::saveLog('UserController', 'actionGetaddresslist', $ip, 'sysloan', $httpParams['user_id']);
		/*		 * *************记录访问日志end**************** */

		$mobileInfo = (new Address_list)->findAllMobile($httpParams['user_id']);
	...
	public function actionGetuserinformation() {
		...
		/*		 * *************记录访问日志beigin************* */
		$ip = Common::get_client_ip();
		$result_log = Common::saveLog('UserController', 'actionGetuserinformation', $ip, 'sysloan', $httpParams['user_id']);
		/*		 * *************记录访问日志end**************** */
		$fuser = (new User())->getUserinfoByUserId($httpParams['user_id']);
		if (empty($fuser)) {
			$array = $this->errorreback('10001');
	...
	private function userresult($code, $object, $loanCount = 0) {
		...
		$data['social'] = (new Selection())->getSocialByUserId($object['user_id'], 2); //社保
		$data['accumulation'] = (new Selection())->getSocialByUserId($object['user_id'], 3); //公积金
