...
class RepayController extends ApiController {
    ...
    public function actionRepaylist() {
        ...
        $parentloanInfo = User_loan_guide::find()->where(['parent_loan_id' => $loanInfo->parent_loan_id])->indexBy('loan_id')->all();
        $loanids  = array_keys($parentloanInfo);


        if (isset($httpParams['offline']) && $httpParams['offline'] == 1) {
            $loanInfo = (new Loan_repay_guide())->getOfflineRepayByLoanId($httpParams['loan_id']);  //获取线下成功还款
        } else {
            $loanInfo = (new Loan_repay_guide)->getRepayByLoanId($loanids);  //获取所有成功还款
        }
...
//
        ...
        $needAmount = (new User_loan)->getRepayment($loanInfo);  //应还款

        $alreadyAmount = $loanInfo->getRepayAmount();
...
//        $renewalNum    = (new User_loan())->getRenewalNum($httpParams['loan_id']);
    ...
    public function actionRepaybeforelist() {
        ...
        $loanInfo = (new Loan_repay_guide())->getBeforeRepay($httpParams['repay_id']);  //获取所有成功还款
        if (empty($loanInfo)) {
            $array = $this->errorreback('60012');
            exit(json_encode($array));
        }
...
//                $data[$key]['is_renewal'] = 1;
    ...
    public function actionNowstatus() {
        ...
        //还款信息
        $repayInfo         = Loan_repay_guide::find()->where(['loan_id' => $httpParams['loan_id'], 'status' => 1])->all();
