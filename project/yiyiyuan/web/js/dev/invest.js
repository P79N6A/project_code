...
$(function(){
	...
	$("#i_know_first").click(function(){
		$(".mLayerMask").css('height','961px');
		$("#i_know_invest_down").hide();
		$("#i_know_invest_bottom").css('display','block'); 
	});
	
	$("#i_know_bottom").click(function(){
		$(".mLayerMask").hide();
	});
	...
	$("#invest_standard_getcode").click(function(){
		...
		$.post("/dev/standard/sendsms",{mobile:mobile,type:16,standard_id:standard_id,invest_share:invest_share},function(result){
			...
			if (data.ret == '0') {
				...
				$("#invest_standard_getcode").attr('disabled', false);
			}
		});
	});
	...
	$("#reback_standard_getcode").click(function(){
		...
		$.post("/dev/standard/sendsms",{mobile:mobile,type:17,reback_share:reback_share},function(result){
			...
			if (data.ret == '0') {
				...
				$("#reback_standard_getcode").attr('disabled', false);
			}
		});
	});
	...
	$("#getcoupon_sendmobile").click(function(){
		...
		$.post("/dev/standard/sendsms",{mobile:mobile,type:18},function(result){
			...
			$("#getcoupon_sendmobile").attr('disabled', false);
		});
	});
	...
	$("#button_getcoupon").click(function(){
		...
		$.post("/dev/standard/getcoupon",{mobile:mobile,code:code,from_user_id:from_user_id,standard_id:standard_id},function(result){
			...
			if (data.ret == '1'){
				...
				window.location = "/dev/standard/couponsuccess?user_id="+from_user_id+"&standard_id="+standard_id+"&mobile="+mobile;
			}
		});
...
var CountDown = function() {
    ...
    $("#invest_standard_getcode").html("重新获取 ( " + count + " ) ");
    $("#send_mobile").html(html);
    if (count <= 0) {
        $("#invest_standard_getcode").html("获取验证码").removeAttr("disabled");
...
var CountDown_back = function() {
    ...
    $("#reback_standard_getcode").html("重新获取 ( " + count + " ) ");
    if (count <= 0) {
        $("#reback_standard_getcode").html("获取验证码").removeAttr("disabled");
        clearInterval(CountDown);
    }
    count--;
};
...
var CountDown_getcoupon = function() {
    ...
    $("#getcoupon_sendmobile").html("重新获取 ( " + count + " ) ");
    if (count <= 0) {
        $("#getcoupon_sendmobile").html("获取验证码").removeAttr("disabled");
        clearInterval(CountDown);
    }
    count--;
};