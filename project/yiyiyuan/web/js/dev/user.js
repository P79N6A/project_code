...
$(function() {
    ...
    $('#reggetcode').click(function() {
        ...
        var app_url = 'http://mp.yaoyuefu.com';
...
//				                    {caption: '取消', callback: function() {}},
    ...
    $('#reggetcode_login').click(function() {
        ...
        $.post("/dev/reg/loginsend", {mobile: mobile, pic_num: pic_num, mark: mark}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                //发送成功
                count = 60;
    ...
    $('#get_bankcode').click(function() {
        ...
        $.post("/dev/bank/banksend", {mobile: mobile, user_id: user_id, cardno: cardno}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                //发送成功
                count = 60;
    ...
    $('#chek').click(function() {
        var money = $('#chek').attr('val');
        var user_id = $('#chek').attr('user');
        $.post("/dev/account/getredpacket", {money: money, user_id: user_id}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret_code == 0) {//成功兑换
                $('#one').hide();
                $('#three').show();
            } else if (data.ret_code == 1) {//用户不存在，提示数据错误
                $('#one').hide();
                $('#errormsg').html(data.ret_msg);
                $('#two').show();

            } else if (data.ret_code == 2) {//兑换金额不正确
                $('#one').hide();
                $('#errormsg').html(data.ret_msg);
                $('#two').show();
    ...
    $("#login_button").click(function() {
        ...
        $.post("/dev/reg/loginsave", {mobile: mobile, code: code, from_code: from_code}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                if (data.url != '') {
...
//                                   alert('sss');
    ...
    $("#regonebutton").click(function() {
        ...
        $(this).attr('disabled', true);
        $.post("/dev/reg/setfromcode", {mobile: mobile, from_code: from_code}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                if (data.url != '') {
                    window.location = data.url;
    ...
    $("#reg_cancle").click(function() {
        ...
        $(this).attr('disabled', true);
        $.post("/dev/reg/canclefromcode", {mobile: mobile}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                if (data.url != '') {
                    window.location = data.url;
    ...
    //存管绑卡
    $("#bank_card_bank").click(function () {
        var code = $("input[name='verifyCodeBank']").val();
        var bank_id = $("input[name='bank_id']").val();
        var mobile = $("input[name='mobile']").val();
        var srvAuthCode = $("input[name='srvAuthCode']").val();
        var csrf = $("#csrfs").val();
        $.post("/new/loan/binding",{srvAuthCode: srvAuthCode,mobile: mobile,code: code,bank_id: bank_id,_csrf:csrf}, function (res) {
            var datas = eval("(" + res + ")");
            if(datas.ret != '0'){
                alert(datas.msg)
            }else{
    ...
    $("#reg_two_form").click(function() {
        ...
        $.post("/dev/reg/twosave", {user_id: user_id, school: school, school_name: school_name, edu: edu, school_time: school_time, realname: realname, identity: identity, from_url: from_url, f_url: f_url}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                if (data.url != '') {
                    window.location = data.url;
    ...
    $("#reg_shtwo_form").click(function() {
        ...
        if (industry == 0 || profession == 0 || position == 0) {
            alert('请选择相关信息');
            return false;
        }
        if (company == '' || address == '' || email == '' || telephone == '') {
            alert("请完整输入信息");
            return false;
        }
        ...
        $("#reg_shtwo_form").attr('disabled', true);
        $.post("/dev/reg/companysave", {email: $.trim(email), telephone: telephone.trim(), company: company, address: address, industry: industry, profession: profession, position: position, income: income, district: district, from_url: from_url, user_id: user_id}, function(result) {
            var data = eval("(" + result + ")");
            ...
            if (data.ret == '0') {
                if (data.url != '') {
                    ...
                    alert('提交失败');
                    $("#reg_shtwo_form").attr('disabled', false);
                    return false;
                }
            } else if (data.ret == '2')
            {
    ...
    $("#reg_shmodifytow_form").click(function() {
        ...
        $.post("/dev/reg/shmodifytowsave", {user_id: user_id, industry: industry, company: company, position: position, from_url: from_url, f_url: f_url}, function(result) {
//			alert(result);
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                if (data.url != '') {
    ...
    $("#reg_shthree_form").click(function() {
        ...
        $.post("/dev/reg/shthreesave", {user_id: user_id, school: school, school_name: school_name, edu: edu, school_time: school_time, f_url: f_url}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                if (data.url != '') {
                    window.location = data.url;
