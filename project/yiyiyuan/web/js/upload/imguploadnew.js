document.domain = "xianhuahua.com";
/**
 * 图片检测程序
 * @param {Object} file
 * @param {Object} fnCall
...
function ImageForm(formid,action,encrypt){
	...
	me.tplInput = '	<div id="!{group}_group"><input id="!{group}_url"  name="!{group}[url]" type="hidden"   value="!{url}">\
								...
								<input id="!{group}_ext" type="hidden" name="!{group}[ext]" value="png">\
...
ImageUpload.prototype.setBase64 = function(id, oFile, fnCall){
	var me = this;
	try{
		var reader = new FileReader(oFile);
		reader.onload = function ( event ) {
			var src = event.target.result;
			// src = src.replace('data:image/png', 'data:image/jpeg');
			console.log(src);
		...
		//	var src = rst.base64;
		//	//var src = src.replace('data:image/jpeg', 'data:image/png');
		//	//console.log(src);
		//	document.getElementById(id+'_base64').value = src;
		//	fnCall = fnCall || me.priview;
		//	fnCall(id, rst, oFile);
