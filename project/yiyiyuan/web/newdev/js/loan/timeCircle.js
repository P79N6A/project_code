	...
	(function($) {
		$.fn.circleCountDown = (function(){
			...
			function CountDown(option){
				...
				this.endTxt = option.endTxt || '';							      // 倒计时结束文字
				this.changeTime = option.changeTime || -1000; 					      // 剩余多长时间改变颜色
				this.changeColor = option.changeColor || '#C90000';				      // 剩余x秒后，变成的颜色
			...
			CountDown.prototype = {
				...
				beforeStart: function(){
					...
					if(this.startRmainTime > 0){
						...
						// 倒计时未开始
						this.oWrap.find(this.oTxt ).html('');
						var startTimer = setInterval(function(){  
							_this.startRmainTime --;
							// console.log(_this.startRmainTime)o
			...
			*/ 
			function createCricle (option){
				option = $.extend({
					...
					color: "#C90000",
					bgColor:"#EAEAEA",
					startProgressPosstartProgressPos: 20
				}, option);
