...
//				                    {caption: '取消', callback: function() {}},
    ...
    $('#reggetcode_login').click(function() {
        ...
        $.post("/new/reg/loginsend", {_csrf:_csrf, mobile: mobile, pic_num: pic_num, mark: mark}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                //发送成功
                count = 60;
    ...
    $("#login_button").click(function() {
        ...
        $.post("/new/reg/loginsave", {_csrf:_csrf, mobile: mobile, code: code, from_code: from_code, come_from: come_from}, function(result) {
            var data = eval("(" + result + ")");
            if(data.ret == '0') {
                if (data.url != '') {
                	if(data.data.status==3){
                        zhuge.identify(data.data.user_id, {
                            '已认证': 1,  // 0表示false，1表示ture，下同
                        });
					}
    ...
    $("#regonebutton").click(function() {
        ...
        $(this).attr('disabled', true);
        $.post("/new/reg/setfromcode", {_csrf:_csrf, mobile: mobile, from_code: from_code}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                if (data.url != '') {
                    window.location = data.url;
    ...
    $("#reg_cancle").click(function() {
        ...
        $(this).attr('disabled', true);
        $.post("/new/reg/canclefromcode", {_csrf:_csrf, mobile: mobile}, function(result) {
            var data = eval("(" + result + ")");
            if (data.ret == '0') {
                if (data.url != '') {
                    window.location = data.url;
